...
int subterfuge(TBeing* thief, TBeing* victim) {
  ...
  if (!victim->isHumanoid() && bKnown < 80) {
    thief->sendTo("You aren't skilled enough to cudgel monsters, yet.\n\r");
    return TRUE;
  }
  ...
  int attackValue = thief->specialAttack(victim, SKILL_SUBTERFUGE, situationalMod, 
                                        ...
                                        STAT_INT, STAT_WIS,  // Defender stats
                                        true);  // Allow partial success

  if (attackValue > FAILURE) {
    // Success - apply base effects
    ...
    // Tier 3: 20% advanced discipline - Remove thief/cityguard/archer procs
    if (advLearning >= 20) {
      REMOVE_BIT(victim->specials.act, ACT_NICE_THIEF);
      REMOVE_BIT(victim->specials.act, ACT_GUARDIAN);
      REMOVE_BIT(victim->specials.act, ACT_AFRAID);
      act("$N seems less vigilant about $S duties.", FALSE, thief, NULL, victim, TO_CHAR);
    }

    TMonster* mob = dynamic_cast<TMonster*>(victim);
    if (mob && (mob->spec == SPEC_CITYGUARD || 
      ...
      act("$N seems less vigilant about $S duties.", FALSE, thief, NULL, mob, TO_CHAR);
    }
    
    // Tier 4: 50% advanced discipline - Remove aggro flag
    if (advLearning >= 50 && IS_SET(victim->specials.act, ACT_AGGRESSIVE)) {
    ...
    // Tier 5: 80% advanced discipline - Remove berserk from players
    if (advLearning >= 80 && victim->isPc() && victim->isCombatMode(ATTACK_BERSERK)) {
...
int skulk(TBeing* thief, spellNumT skill) {
  ...
  if (thief->affectedBySpell(SPELL_SKULK)) {
  ...
  // Start the task
  start_task(thief, skulk_task, NULL, NULL, taskTime, thief->in_room, 0, 0, 0);
