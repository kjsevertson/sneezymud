...
int forage(TBeing* caster, short bKnown) {
  ...
  if (!caster->roomp) {
    vlogf(LOG_BUG,
      format("Forage called without room pointer: %s") % caster->name);
    return SPELL_FAIL;
  }
  ...
  if (caster->roomp->isCitySector() ||
    ...
    return SPELL_FAIL;
             ...
             caster->roomp->isAirSector() || caster->roomp->isOceanSector() ||
             caster->roomp->isRiverSector()) {
    // being safe here - allowing arctic allows a bunch of undesired sector
    // types
    ...
    caster->sendTo(
      "The raging fire here needs to be controlled before you can forage.\n\r");
    return SPELL_FAIL;
  }
...
static const insectForage terrain_insects[MAX_SECTOR_TYPES] = {
  ...
  {1.75, 2,
    {"daddy long legs", "hoary mosquito"}},  // SECT_TROPICAL_RIVER_SURFACE,
...
int forage_insect(TBeing* caster) {
  ...
  if (caster->doesKnowSkill(caster->getSkillNum(SKILL_FORAGE)) &&
      caster->bSuccess(SKILL_FORAGE))
    food *=
      (int(caster->getSkillValue(caster->getSkillNum(SKILL_FORAGE)) + 50) / 50);
  ...
  for (StuffIter it = caster->roomp->stuff.begin();
    ...
    if (dynamic_cast<TTrash*>(*it) ||
        (dynamic_cast<TFood*>(*it) &&
          dynamic_cast<TFood*>(*it)->isFoodFlag(FOOD_SPOILED))) {
      food += 0.5;
      break;
    }
  }
  ...
  static const sstring bugsizes[] = {"miniscule", "tiny", "juicy", "fat",
    "giant", "gargantuan", "HUMONGOUS"};
  static const sstring actions[] = {"quickly gobbles up a %s",
    "crunches down a %s", "swallows a %s in a single gulp",
    "tears up a %s and eats it"};

  sstring bugname = bugsizes[min(
    max(0, int(food * int(cElements(bugsizes) - 1) / (3 * FORAGE_INSECT_FOOD))),
    int(cElements(bugsizes) - 1))];
  bugname += " ";
  bugname += bug;
...
int determineSkinningItem(TBaseCorpse* corpse, int* amount, char* msg,
  char* gl_msg) {
  int num = -1;
...
void TThing::skinMe(TBeing* ch, const char* arg) {
  ...
  // bonus turns to complete or penantly turns.  Then we make sure that is all
  // higher than 1, then add 2 onto that.  This way even a rock bottom example
  // will have at least 3 turns to complete a skin.
  // ***This has been changed so don't rely on the comments, will redo them when
  // I finalize
  // ***this formula.  -Lapsos
...
void TTool::skinMe(TBeing* ch, const char* arg) {
  ...
  if (getToolType() != TOOL_SKIN_KNIFE) {
    ch->sendTo(
      "You must be holding a skinning knife to perform this task.\n\r");
    return;
  }
  // Check to see if argument passed exists in room
  if (!generic_find(arg, FIND_OBJ_ROOM, ch, &dummy, &obj)) {
...
void TBeing::doSeekwater() {
  ...
  if (affectedBySpell(SKILL_TRACK) || specials.hunting) {
    sendTo(
      "You can't search for water while tracking something else as well.\n\r");
    return;
  }
          ...
          if (tp) {
            ...
            if (count == seen) {
              sendTo(COLOR_OBJECTS,
                format("%sYou see traces of water through %s.%s\n\r") %
...
int encamp(TBeing* caster) {
  ...
  if (caster->roomp->isRoomFlag(ROOM_ON_FIRE)) {
    caster->sendTo(
      "The raging fire here needs to be controlled before setting up "
      "camp.\n\r");
    return SPELL_FAIL;
  }
  if (caster->roomp->isRoomFlag(ROOM_FLOODED)) {
    caster->sendTo(
      "The flood in here needs to subside before setting up camp.\n\r");
    return SPELL_FAIL;
  }
  ...
  if (caster->roomp->isFlyingSector() || caster->roomp->isVertSector() ||
      caster->roomp->isUnderwaterSector() || caster->roomp->isAirSector() ||
      caster->roomp->isOceanSector() || caster->roomp->isRiverSector()) {
...
void divine(TBeing* caster, TThing* obj) {
  divine(caster, caster->getSkillLevel(SKILL_DIVINATION),
    caster->getSkillValue(SKILL_DIVINATION), obj);
}
...
int TDrinkCon::divineMe(TBeing* caster, int, short bKnown) {
  ...
  if (!(caster->roomp->isForestSector() || caster->roomp->isBeachSector() ||
        caster->roomp->isHillSector() || caster->roomp->isMountainSector() ||
        caster->roomp->isNatureSector() || caster->roomp->isRoadSector() ||
    ...
    caster->sendTo(
      "The flood in here needs to subside before you can divine.\n\r");
    return SPELL_FAIL;
  } else if (caster->roomp->isRoomFlag(ROOM_ON_FIRE)) {
    caster->sendTo(
      "The raging fire here needs to be controlled before you can divine.\n\r");
    return SPELL_FAIL;
  }
