...
static int injureLimbs(TBeing* ch, TBeing* victim, spellNumT skill, int level,
  ...
  act("$d aids you to a lesser extent, damaging $N's limbs!", FALSE, ch, 0,
    victim, TO_CHAR);
  act("$d blasts you, damaging your limbs!", FALSE, ch, 0, victim, TO_VICT);
  act("$d blasts $N, damaging $S limbs!", FALSE, ch, 0, victim, TO_NOTVICT);
...
int harm(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), spell)) {
    ...
    switch (critFail(caster, spell)) {
      ...
      case CRIT_F_HITSELF:
        CF(spell);
        act("Something goes tragically wrong and $n buckles in pain!", FALSE,
          caster, NULL, NULL, TO_ROOM);
        act("Something goes tragically wrong and you buckle in pain!", FALSE,
          caster, NULL, NULL, TO_CHAR);
        act("That pain was intended for you!", FALSE, caster, NULL, victim,
          TO_VICT);
...
int harm(TBeing* caster, TBeing* victim, TMagicItem* obj, spellNumT spell) {
  ...
  act("$p invokes an injurious prayer against you.", FALSE, victim, obj, 0,
    TO_CHAR);
  act("$p invokes an injurious prayer against $n.", FALSE, victim, obj, 0,
    TO_ROOM);

  ret = harm(caster, victim, obj->getMagicLevel(), obj->getMagicLearnedness(),
    spell, 0);
...
int harm(TBeing* caster, TBeing* victim) {
  ...
  if (caster != victim) {
    act("You invoke an injurious prayer against $N.", FALSE, caster, 0, victim,
      TO_CHAR);
    act("$n invokes an injurious prayer against you.", FALSE, caster, 0, victim,
      TO_VICT);
    act("$n invokes an injurious prayer against $N.", FALSE, caster, 0, victim,
      TO_NOTVICT);
  } else {
    act("You invoke an injurious prayer against yourself.", FALSE, caster, 0, 0,
      TO_CHAR);
    act("$n invokes an injurious prayer against $mself.", FALSE, caster, 0, 0,
      TO_ROOM);
  }

  int ret = harm(caster, victim, level, caster->getSkillValue(spell), spell,
    caster->getAdvLearning(spell));
...
int poison(TBeing* caster, TObj* target, TMagicItem* obj, spellNumT spell) {
  ...
  ret = poison(caster, target, obj->getMagicLevel(), obj->getMagicLearnedness(),
    spell);
  if (ret == SPELL_SUCCESS) {
...
int poison(TBeing* caster, TBeing* victim, int level, short bKnown,
  spellNumT spell) {
  ...
  if (caster->isNotPowerful(victim, level, spell, SILENT_YES)) {
    ...
    act("It looks like you didn't pray hard enough!", FALSE, caster, NULL,
      victim, TO_CHAR);
    act("$n just tried to poison you!  Luckily, $e is too feeble.", FALSE,
      caster, NULL, victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, 0, victim,
      TO_NOTVICT);
    return SPELL_FALSE;
  }

  if (victim->isImmune(IMMUNE_POISON, WEAR_BODY)) {
    act("Your pious poison seems to have no effect on $N!", FALSE, caster, NULL,
      victim, TO_CHAR);
    act("$n just tried to poison you!  Luckily, you are immune.", FALSE, caster,
      NULL, victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, 0, victim,
      TO_NOTVICT);
    return SPELL_FALSE;
  ...
  if (victim->affectedBySpell(SPELL_POISON) ||
    ...
    act("You can't poison $N.  $E is already poisoned!", FALSE, caster, NULL,
      victim, TO_CHAR);
    act("$n just tried to poison you again!", FALSE, caster, NULL, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, 0, victim,
      TO_NOTVICT);
    return SPELL_FALSE;
...
void poison(TBeing* caster, TBeing* victim) {
  ...
  if (IS_SET(ret, SPELL_SUCCESS)) {
    if (caster != victim) {
      sprintf(gender_desc, "%s",
        (!caster->getSex() ? "eunuch"
                           : (caster->getSex() == 1 ? "bastard" : "bitch")));
      ...
      act("You fill $N's veins with poison!", FALSE, caster, NULL, victim,
        TO_CHAR);
    } else {
      caster->sendTo("Way to go!  You've poisoned yourself!!\n\r");
      act("$n looks sheepish as $e realizes $e just poisoned $mself.", FALSE,
        caster, 0, 0, TO_ROOM);
    }
    ...
    if (caster != victim) {
      ...
      act(buf, FALSE, caster, NULL, victim, TO_VICT);
      act("You fill $N's veins with a serious poison!", FALSE, caster, NULL,
        victim, TO_CHAR);
    } else {
      act("Way to go!  You've poisoned yourself badly!", FALSE, caster, NULL,
        NULL, TO_VICT);
      act("$n looks sheepish as $e realizes $e just poisoned $mself seriously.",
        FALSE, caster, 0, 0, TO_ROOM);
    }
  } else if (IS_SET(ret, SPELL_SAVE)) {
  } else if (IS_SET(ret, SPELL_CRIT_FAIL)) {
    act("$n seems to have poisoned $mself!", FALSE, caster, NULL, caster,
      TO_NOTVICT);
    act("Oh no!  You seem to have poisoned yourself!", FALSE, caster, NULL,
      NULL, TO_CHAR);
    act("That poison was intended for you!", FALSE, caster, NULL, victim,
      TO_VICT);
...
int poison(TBeing* caster, TBeing* victim, TMagicItem* obj, spellNumT spell) {
  ...
  ret = poison(caster, victim, obj->getMagicLevel(), obj->getMagicLearnedness(),
    spell);
  if (IS_SET(ret, SPELL_SUCCESS)) {
    if (caster == victim) {
      ...
      act("Poison courses through your veins!", FALSE, victim, 0, 0, TO_CHAR);
      act("Poison courses through $n's veins!", FALSE, victim, 0, 0, TO_ROOM);
    }
  } else if (IS_SET(ret, SPELL_CRIT_SUCCESS)) {
    if (caster == victim) {
      caster->sendTo("Don't you feel stupid for poisoning yourself?\n\r");
    ...
    act("Oops!  It backfired on you and now you're poisoned!", FALSE, caster, 0,
      victim, TO_CHAR);
    act("$n just tried to poison you!", FALSE, caster, 0, victim, TO_VICT);
    act("The funny part is, $e poisoned $mself instead!", FALSE, caster, 0,
      victim, TO_VICT);
    act("$n fails miserably and poisons $mself.", FALSE, caster, 0, victim,
      TO_NOTVICT);
  } else if (IS_SET(ret, SPELL_FAIL)) {
    act("Nothing seems to happen.  You fail to invoke the poison.", FALSE,
      caster, 0, 0, TO_CHAR);
...
int blindness(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->isNotPowerful(victim, level, SPELL_BLINDNESS, SILENT_YES)) {
    ...
    act("Alas, your prayer is too feeble.", FALSE, caster, NULL, victim,
      TO_CHAR);
    act("$n just tried to blind you!  Luckily, $e is too weak.", FALSE, caster,
      NULL, victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, 0, victim,
      TO_NOTVICT);
    return SPELL_FALSE;
  ...
  if (victim->isAffected(AFF_TRUE_SIGHT) || victim->isAffected(AFF_CLARITY)) {
    act("$N's vision is much too clear to be blinded!", FALSE, caster, NULL,
      victim, TO_CHAR);
    act("$n just tried to blind you, but your vision is too clear!", 0, caster,
      NULL, victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, 0, victim,
      TO_NOTVICT);
    return SPELL_FALSE;
  ...
  if (victim->affectedBySpell(SPELL_BLINDNESS)) {
    act("You can't blind $N since $E is already blind!", FALSE, caster, NULL,
      victim, TO_CHAR);
    act("$n just tried to blind you again!", FALSE, caster, NULL, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, 0, victim,
      TO_NOTVICT);
    return SPELL_FALSE;
...
void blindness(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  if (IS_SET(ret, SPELL_SUCCESS) && !IS_SET(ret, SPELL_CRIT_SUCCESS)) {
    if (caster != victim) {
      act("A thin cloudy film appears over $N's eyes.", FALSE, caster, NULL,
        victim, TO_CHAR);
      act("A thin cloudy film appears over $N's eyes.", FALSE, caster, NULL,
        victim, TO_NOTVICT);
      act("A thin cloudy film appears over your eyes.", FALSE, caster, NULL,
        victim, TO_VICT);
      act("$n has used $p to blind $N -- $E's virtually helpless!", FALSE,
        caster, obj, victim, TO_NOTVICT);
      act("You have used $p to blind $N -- $E's virtually helpless!", FALSE,
        caster, obj, victim, TO_CHAR);
      act("$n has used $p to blind you!  You're virtually helpless!", FALSE,
        caster, obj, victim, TO_VICT);
    } else {
      act("A thin cloudy film appears over your eyes.", FALSE, caster, NULL,
        NULL, TO_CHAR);
      act("A thin cloudy film appears over $n's eyes.", FALSE, caster, NULL,
      ...
      act("$n has used $p to blind $mself -- $e's virtually helpless!", FALSE,
        caster, obj, NULL, TO_ROOM);
    }
    victim->rawBlind(level, duration, save);
  } else if (IS_SET(ret, SPELL_CRIT_SUCCESS)) {
    if (caster != victim) {
      act("A dense dark film appears over $N's eyes.", TRUE, caster, NULL,
        victim, TO_CHAR);
      act("A dense dark film appears over $N's eyes.", TRUE, caster, NULL,
        victim, TO_NOTVICT);
      act("A dense dark film appears over your eyes.", FALSE, caster, NULL,
        victim, TO_VICT);
      act("$n has used $p to blind $N -- $E's virtually helpless!", TRUE,
        caster, obj, victim, TO_NOTVICT);
      act("You have used $p to blind $N -- $E's virtually helpless!", FALSE,
        caster, obj, victim, TO_CHAR);
      act("$n has used $p to blind you!  You're virtually helpless!", FALSE,
      ...
      act("A dense dark film appears over your eyes.", TRUE, caster, NULL, NULL,
        TO_CHAR);
      act("A dense dark film appears over $n's eyes.", TRUE, caster, NULL, NULL,
        TO_ROOM);
      act("You have used $p to blind yourself -- you're virtually helpless!",
        FALSE, caster, obj, NULL, TO_CHAR);
      act("$n has used $p to blind $mself -- $e's virtually helpless!", TRUE,
        caster, obj, NULL, TO_ROOM);
    }
    duration *= 2;
    victim->rawBlind(level, duration, save);
  } else if (IS_SET(ret, SPELL_FAIL)) {
    act("Poor $n can't even figure out how to use $p right!", FALSE, caster,
      obj, victim, TO_NOTVICT);
    act("$N doesn't seem to be blinded by $p.", TRUE, caster, obj, victim,
      TO_CHAR);
    act("$n was trying to harm you with $p!", TRUE, caster, obj, victim,
      TO_VICT);

    if (IS_SET(ret, SPELL_CRIT_FAIL)) {
      act("$n seems to have blinded $mself with $p!", FALSE, caster, obj, NULL,
        TO_ROOM);
      act("Ack, you seem to have blinded yourself with $p.", FALSE, caster, obj,
        NULL, TO_CHAR);
...
void blindness(TBeing* caster, TBeing* victim) {
  ...
  int ret =
    blindness(caster, victim, level, caster->getSkillValue(SPELL_BLINDNESS));
  ...
  if (IS_SET(ret, SPELL_SUCCESS) && !IS_SET(ret, SPELL_CRIT_SUCCESS)) {
    act("A thin cloudy film appears over $N's eyes.", TRUE, caster, NULL,
      victim, TO_CHAR);
    act("A thin cloudy film appears over $N's eyes.", TRUE, caster, NULL,
      victim, TO_NOTVICT);
    act("A thin cloudy film appears over your eyes.", FALSE, caster, NULL,
      victim, TO_VICT);
    act("$n has blinded $N -- $E's virtually helpless!", TRUE, caster, NULL,
      victim, TO_NOTVICT);
    act("You have blinded $N -- $E's virtually helpless!", FALSE, caster, NULL,
      victim, TO_CHAR);
    act("$n has blinded you!  You're virtually helpless!", FALSE, caster, NULL,
      victim, TO_VICT);
    victim->rawBlind(level, duration, save);
  } else if (IS_SET(ret, SPELL_CRIT_SUCCESS)) {
    act("A dense dark film appears over $N's eyes.", TRUE, caster, NULL, victim,
      TO_CHAR);
    act("A dense dark film appears over $N's eyes.", TRUE, caster, NULL, victim,
      TO_NOTVICT);
    act("A dense dark film appears over your eyes.", FALSE, caster, NULL,
    ...
    act("Poor $n can't even invoke a little prayer!", FALSE, caster, NULL,
      victim, TO_NOTVICT);
    act("Hmm, your attempt to blind $N failed.", FALSE, caster, NULL, victim,
      TO_CHAR);
    act("$n was trying to harm you with a prayer!", FALSE, caster, NULL, victim,
      TO_VICT);

    if (IS_SET(ret, SPELL_CRIT_FAIL)) {
      act("$n seems to have blinded $mself!", FALSE, caster, NULL, NULL,
        TO_ROOM);
      act("Ack, you seem to have blinded yourself.", FALSE, caster, NULL, NULL,
        TO_CHAR);
      caster->rawBlind(level, duration, SAVE_YES);
    }
  }
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), spell)) {
    ...
    switch (critFail(caster, spell)) {
      ...
      case CRIT_F_HITSELF:
        CF(spell);
        act(
          "Something goes tragically wrong and you have lightly hurt yourself!",
          FALSE, caster, NULL, NULL, TO_CHAR);
        act("Something goes tragically wrong and $n has lightly hurt $mself!",
          FALSE, caster, NULL, NULL, TO_ROOM);
        act("$n's injury was intended for you!", FALSE, caster, NULL, victim,
          TO_VICT);
        if (caster->reconcileDamage(caster, dam, spell) == -1)
          return SPELL_CRIT_FAIL + CASTER_DEAD;
        return SPELL_CRIT_FAIL;
...
int harmLight(TBeing* caster, TBeing* victim, TMagicItem* obj,
  spellNumT spell) {
  int ret, rc = 0;
  ...
  ret = harmLight(caster, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), spell, 0);
...
int harmLight(TBeing* caster, TBeing* victim) {
  ...
  int ret = harmLight(caster, victim, level, caster->getSkillValue(spell),
    spell, caster->getAdvLearning(spell));
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else if (IS_SET(ret, SPELL_CRIT_FAIL)) {
  } else {
  }
...
int harmCritical(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), spell)) {
    ...
    if (critFail(caster, spell) != CRIT_F_NONE) {
      CF(spell);
      act(
        "Something goes tragically wrong and you have critically hurt "
        "yourself!",
        FALSE, caster, NULL, NULL, TO_CHAR);
...
int harmCritical(TBeing* caster, TBeing* victim, TMagicItem* obj,
  spellNumT spell) {
  int ret, rc = 0;
  ...
  ret = harmCritical(caster, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), spell, 0);
...
int harmCritical(TBeing* caster, TBeing* victim) {
  ...
  int ret = harmCritical(caster, victim, level, caster->getSkillValue(spell),
    spell, caster->getAdvLearning(spell));
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else if (IS_SET(ret, SPELL_CRIT_FAIL)) {
  } else {
...
int harmSerious(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), spell)) {
    ...
    if (critFail(caster, spell) != CRIT_F_NONE) {
      CF(spell);
      act(
        "Something goes tragically wrong and you have seriously hurt yourself!",
        FALSE, caster, NULL, NULL, TO_CHAR);
...
int harmSerious(TBeing* caster, TBeing* victim, TMagicItem* obj,
  spellNumT spell) {
  int ret, rc = 0;
  ...
  ret = harmSerious(caster, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), spell, 0);
...
int harmSerious(TBeing* caster, TBeing* victim) {
  ...
  int ret = harmSerious(caster, victim, level, caster->getSkillValue(spell),
    spell, caster->getAdvLearning(spell));
  if (IS_SET(ret, SPELL_SUCCESS)) {
  } else if (IS_SET(ret, SPELL_CRIT_FAIL)) {
  } else {
...
int paralyze(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->isNotPowerful(victim, level, SPELL_PARALYZE, SILENT_YES)) {
    act("Your prayer is feeble and $N is unaffected!", FALSE, caster, NULL,
      victim, TO_CHAR);
    act("$n just tried to paralyze you!  Luckily, $e is too weak.", FALSE,
      caster, NULL, victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, NULL,
      victim, TO_NOTVICT);
    return SPELL_FALSE;
  }
  if (victim->isUndead() || victim->isImmune(IMMUNE_PARALYSIS, WEAR_BODY)) {
    act("$d responds to your prayer, but $N does not!", FALSE, caster, NULL,
      victim, TO_CHAR);
    act("$n just tried to paralyze you.  That tickled!", FALSE, caster, NULL,
      victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, NULL,
      victim, TO_NOTVICT);
    return SPELL_FALSE;
  ...
  if (victim->isAffected(AFF_PARALYSIS)) {
    act("Try paralyzing someone who can still move.", FALSE, caster, NULL,
      victim, TO_CHAR);
    act("$n must really want you to be paralyzed!", FALSE, caster, NULL, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, NULL,
      victim, TO_NOTVICT);
    return SPELL_FALSE;
...
void paralyze(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  ret =
    paralyze(caster, victim, obj->getMagicLevel(), obj->getMagicLearnedness());
  if (IS_SET(ret, SPELL_SUCCESS)) {
    act("A thin cloud of icy air has surrounded $N!", FALSE, caster, NULL,
      victim, TO_NOTVICT);
    act("A thin cloud of icy air has you surrounded you!", FALSE, caster, NULL,
      victim, TO_VICT);
    act("$N falls down!", FALSE, caster, NULL, victim, TO_NOTVICT);
    act("$N falls down!", FALSE, caster, NULL, victim, TO_CHAR);
    act("$N stops moving -- $n has paralyzed $M!", FALSE, caster, NULL, victim,
      TO_NOTVICT);
    act("$N stops moving -- you have paralyzed $M!", FALSE, caster, NULL,
      victim, TO_CHAR);
    ...
    act("A thin cloud of icy air has you surrounded you!", FALSE, caster, NULL,
      victim, TO_VICT);
    act("$d takes pity on $m, and $e breaks free of the paralysis!", FALSE,
      victim, NULL, NULL, TO_ROOM);
    act("$d takes pity on you, and you break free of the paralysis!", FALSE,
      victim, NULL, NULL, TO_CHAR);
  } else if (IS_SET(ret, SPELL_CRIT_SUCCESS)) {
    act("A large icy white cloud of air has totally encased $N!", FALSE, caster,
      NULL, victim, TO_NOTVICT);
    act("A large icy white cloud of air has totally encased you!", FALSE,
      caster, NULL, victim, TO_VICT);
    act("$N falls down!", FALSE, caster, NULL, victim, TO_NOTVICT);
    act("$N falls down!", FALSE, caster, NULL, victim, TO_CHAR);
    act("$N stops moving -- $n has paralyzed $M!", FALSE, caster, NULL, victim,
    ...
    act("Your prayer fails and you are unable to paralyze $N.", FALSE, caster,
      NULL, victim, TO_CHAR);
    caster->deityIgnore(SILENT_YES);
  } else if (IS_SET(ret, SPELL_CRIT_FAIL)) {
    act("$n just paralyzed $mself!", FALSE, caster, NULL, victim, TO_NOTVICT);
    act("Ack!  The affects of the prayer have misfired.", FALSE, caster, NULL,
      victim, TO_CHAR);
    act("Your limbs freeze in place!", FALSE, caster, NULL, victim, TO_CHAR);
    act("$n was trying to paralyze you but got $mself instead!", FALSE, caster,
      NULL, victim, TO_VICT);
  } else if (IS_SET(ret, SPELL_FALSE)) {
...
void paralyze(TBeing* caster, TBeing* victim) {
  ...
  if (IS_SET(ret, SPELL_SUCCESS)) {
    act("A thin cloud of icy air surrounds $n!", FALSE, victim, NULL, NULL,
      TO_ROOM);
    act("A thin cloud of icy air surrounds you!", FALSE, victim, NULL, NULL,
      TO_CHAR);
    act("$N falls down!", FALSE, caster, NULL, victim, TO_NOTVICT);
    act("$N falls down!", FALSE, caster, NULL, victim, TO_CHAR);
    act("$N stops moving -- $n has paralyzed $M!", FALSE, caster, NULL, victim,
  ...
  if (IS_SET(ret, SPELL_CRIT_SUCCESS)) {
    act("A large icy white cloud of air totally encases $n!", FALSE, victim,
      NULL, NULL, TO_ROOM);
    act("A large icy white cloud of air totally encases you!", FALSE, victim,
      NULL, NULL, TO_CHAR);
    act("$N falls down!", FALSE, caster, NULL, victim, TO_NOTVICT);
    act("$N falls down!", FALSE, caster, NULL, victim, TO_CHAR);
    act("$N stops moving -- $n has paralyzed $M!", FALSE, caster, NULL, victim,
  ...
  if (IS_SET(ret, SPELL_FAIL)) {
    act("You prayer fails and you are unable to paralyze $N.", FALSE, caster,
      NULL, victim, TO_CHAR);
    caster->deityIgnore(SILENT_YES);
  }
  if (IS_SET(ret, SPELL_CRIT_FAIL)) {
    act("$n just paralyzed $mself!", FALSE, caster, NULL, victim, TO_NOTVICT);
    act("Ack!  Your prayer backfired on you!", FALSE, caster, NULL, victim,
      TO_CHAR);
    act("Your limbs freeze in place!", FALSE, caster, NULL, victim, TO_CHAR);
    act("$n was trying to paralyze you but got $mself instead!  Ha!", FALSE,
      caster, NULL, victim, TO_VICT);
  }
  if (IS_SET(ret, SPELL_FALSE)) {}
...
bool notBreakSlot(wearSlotT slot, bool avoid) {
  if (slot == WEAR_HEAD || slot == WEAR_NECK || slot == WEAR_BACK ||
      slot == WEAR_BODY || slot == WEAR_WAIST || slot == HOLD_RIGHT ||
      slot == HOLD_LEFT)
    return TRUE;
  else if (!avoid)
    return FALSE;
...
bool TBeing::canBoneBreak(TBeing* victim, silentTypeT silent) {
  if (victim->raceHasNoBones()) {
    if (!silent) {
      act("$N has no bones of note worth breaking.", FALSE, this, 0, victim,
        TO_CHAR);
  ...
  if (checkForTorment(victim)) {
    if (!silent) {
      act("$d refuses to torment $N any further.", FALSE, this, 0, victim,
        TO_CHAR);
      deityIgnore(SILENT_YES);
    }
    return FALSE;
  ...
  if (!ok) {
    act("$N doesn't have a limb left to break!", FALSE, this, NULL, victim,
      TO_CHAR);
    act("Your attack goes for naught.", FALSE, this, NULL, victim, TO_CHAR);
    act("$n just tried to break your bones!", FALSE, this, NULL, victim,
      TO_VICT);
    act("Luckily, you have no limbs to break.", FALSE, this, NULL, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, this, NULL, victim,
      TO_NOTVICT);
    return FALSE;
  }
  if (found) {
    act("$d refuses to break another of $N's limbs!", FALSE, this, NULL, victim,
      TO_CHAR);
    act("Your attack goes for naught.", FALSE, this, NULL, victim, TO_CHAR);
    act("$n just tried to break another bone!", FALSE, this, NULL, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, this, NULL, victim,
      TO_NOTVICT);
    return FALSE;
...
int boneBreaker(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (caster->isNotPowerful(victim, level, SPELL_BONE_BREAKER, SILENT_YES)) {
    act("You invoke $d to break $N's bones!", FALSE, caster, NULL, victim,
      TO_CHAR);
    act("You did not pray hard enough.", FALSE, caster, NULL, victim, TO_CHAR);
    act("$n just tried to break your bones!  Luckily, $e is too feeble.", FALSE,
      caster, NULL, victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, NULL,
      victim, TO_NOTVICT);
    return SPELL_FALSE;
  ...
  if (victim->isImmune(IMMUNE_BONE_COND, slot)) {
    act("You pray for $N's broken bones!  And they do not break.", FALSE,
      caster, NULL, victim, TO_CHAR);
    act("$n just tried to break your unbreakable bones!", FALSE, caster, NULL,
      victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, NULL,
      victim, TO_NOTVICT);
    return SPELL_FALSE;
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), SPELL_BONE_BREAKER)) {
    ...
    if (victim->awake())
      sprintf(buf,
        "You hear a muffled SNAP as $n clutches $s %s in extreme pain!", limb);
    else
    ...
    switch (critSuccess(caster, SPELL_BONE_BREAKER)) {
      ...
      case CRIT_S_DOUBLE:
        ...
        if (!victim->isImmune(IMMUNE_PARALYSIS, slot)) {
          ...
          sprintf(buf,
            "In fact, you can't even move your %s!  It may be paralyzed!",
            limb);
          act(buf, FALSE, caster, NULL, victim, TO_VICT);
          sprintf(buf,
            "$n's %s is hanging uselessly!  It's been paralyzed as well!",
            limb);
          act(buf, FALSE, victim, NULL, NULL, TO_ROOM);
          ret = SPELL_CRIT_SUCCESS;
        }
    ...
    switch (critFail(caster, SPELL_BONE_BREAKER)) {
      ...
      case CRIT_F_HITSELF:
        ...
        if (!caster->isImmune(IMMUNE_BONE_COND, slot)) {
          act("Your prayer backfires on you!", FALSE, caster, NULL, NULL,
            TO_CHAR);
          sprintf(limb, "%s", caster->describeBodySlot(slot).c_str());
          caster->addToLimbFlags(slot, PART_BROKEN);
          sprintf(buf,
            "Extreme pain shoots through your %s!\n\rYour %s has been broken!",
            limb, limb);
          act(buf, FALSE, caster, NULL, NULL, TO_CHAR);
          sprintf(buf,
            "You hear a muffled SNAP as $n clutches $s %s in extreme pain!",
            limb);
    ...
    if (!::number(0, 2)) {
      ...
      int rc =
        injureLimbs(caster, victim, SPELL_BONE_BREAKER, level, adv_learn);
...
int boneBreaker(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  ret = boneBreaker(caster, victim, obj->getMagicLevel(),
  ...
  if (IS_SET(ret, SPELL_SUCCESS)) {
    ...
    act("$p fails to break any of $N's bones!", FALSE, caster, obj, victim,
      TO_CHAR);
    act("$n's $o fails to break any of your bones!", FALSE, caster, obj, victim,
      TO_VICT);
    act("$n's $o fails to break any of $N's bones!", FALSE, caster, obj, victim,
      TO_NOTVICT);
  } else {
    // ??
  }
...
int boneBreaker(TBeing* caster, TBeing* victim) {
  ...
  ret = boneBreaker(caster, victim, level,
    caster->getSkillValue(SPELL_BONE_BREAKER),
    caster->getAdvLearning(SPELL_BONE_BREAKER));
  if (IS_SET(ret, SPELL_SUCCESS)) {
    if (!victim->isPc()) {
      dynamic_cast<TMonster*>(victim)->addHated(caster);
    }
    ...
    act("Your prayer fails to break any of $N's bones!", FALSE, caster, NULL,
      victim, TO_CHAR);
    act("$n's prayer fails to break any of your bones!", FALSE, caster, NULL,
      victim, TO_VICT);
    act("$n's prayer fails to break any of $N's bones!", FALSE, caster, NULL,
      victim, TO_NOTVICT);
  } else {
    // ??
  }
...
int bleed(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (caster->isNotPowerful(victim, level, SPELL_BLEED, SILENT_YES) ||
    ...
    act("You pray for $N's blood, but you do not pray hard enough.", FALSE,
      caster, NULL, victim, TO_CHAR);
    act("$n just tried to bleed your bloodless body!", FALSE, caster, NULL,
      victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, NULL,
      victim, TO_NOTVICT);
    return SPELL_FAIL;
  ...
  if (victim->isUndead()) {
    act("You pray for $N's blood, but $E doesn't seem to have any.", FALSE,
      caster, NULL, victim, TO_CHAR);
    act("$n just tried to bleed your bloodless body!", FALSE, caster, NULL,
      victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, NULL,
  ...
  if (victim->isImmune(IMMUNE_BLEED, WEAR_BODY)) {
    act("You try to bleed $N, but $S veins are like iron!", FALSE, caster, NULL,
      victim, TO_CHAR);
    act("$n just tried to bleed you!  Luckily, you are immune.", FALSE, caster,
      NULL, victim, TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, NULL,
      victim, TO_NOTVICT);
    return SPELL_FAIL;
  ...
  if (slot < MAX_WEAR) {
    ...
    act("$n just tried to bleed you again!", FALSE, caster, NULL, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, NULL,
      victim, TO_NOTVICT);
    return SPELL_FAIL;
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), SPELL_BLEED)) {
    ...
    while (::number(0, 999) < bloodfest) {
      sprintf(limb, "%s", victim->describeBodySlot(slot).c_str());
      sprintf(buf,
        "A gaping gash opens up on your %s and <R>bright<1> <r>red blood<1> "
        "begins to course out!",
        limb);
      act(buf, FALSE, victim, NULL, NULL, TO_CHAR);
      sprintf(buf,
        "The flesh on $n's %s opens up and begins to bleed profusely!", limb);
      act(buf, FALSE, victim, NULL, NULL, TO_ROOM);
...
int bleed(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  FALSE, caster, NULL, victim, TO_VICT); act("What luck!  $p also managed to
  infect it!", FALSE, caster, obj, NULL, TO_CHAR); if (!victim->isPc()) {
      dynamic_cast<TMonster *>(victim)->addHated(caster);
    }
  } */
  else if (IS_SET(ret, SPELL_SAVE)) {
    ...
    act("You see red as $p betrays you and you begin to bleed.", FALSE, caster,
      obj, 0, TO_CHAR);
    act("$n is in shock as the blood begins to course from $s body.", FALSE,
      caster, 0, 0, TO_ROOM);
  } else if (IS_SET(ret, SPELL_FAIL)) {
    if (!victim->isPc()) {
      dynamic_cast<TMonster*>(victim)->addHated(caster);
...
int bleed(TBeing* caster, TBeing* victim) {
  ...
  /* removed crit success in favor of multiple bleed chances
    ...
    act("Oh no!  There are living creatures in the wound!  It's infected!",
  FALSE, caster, NULL, victim, TO_VICT); act("What luck!  Your spell also
  managed to infect it!", FALSE, caster, NULL, NULL, TO_CHAR); if
  (!victim->isPc()) { dynamic_cast<TMonster *>(victim)->addHated(caster);
    }
  } */
  else if (IS_SET(ret, SPELL_SAVE)) {
    if (!victim->isPc()) {
    ...
    act("You see red as your prayer backfires and you begin to bleed.", FALSE,
      caster, 0, 0, TO_CHAR);
    act("$n is in shock as blood begins to course from $s body.", FALSE, caster,
      0, 0, TO_ROOM);
  } else if (IS_SET(ret, SPELL_FAIL)) {
    if (!victim->isPc()) {
      dynamic_cast<TMonster*>(victim)->addHated(caster);
...
bool TBeing::canWither(TBeing* victim, silentTypeT silent) {
  if (victim->isUndead()) {
    if (!silent) {
      act("Your attempt to wither $N is futile!", FALSE, this, NULL, victim,
        TO_CHAR);
      act("$n thinks $e can wither you!", FALSE, this, NULL, victim, TO_VICT);
      act("$n's prayer seems to have no effect on $N.", FALSE, this, NULL,
        victim, TO_NOTVICT);
    }
    return FALSE;
  }
  if (checkForTorment(victim)) {
    if (!silent) {
      act("$d refuses to torment $N any further.", FALSE, this, NULL, victim,
        TO_CHAR);
  ...
  if (!ok) {
    if (!silent) {
      act("$N has no limbs you can wither!", FALSE, this, NULL, victim,
        TO_CHAR);
      act("Your attack goes for naught.", FALSE, this, NULL, victim, TO_CHAR);
      act("$n just tried to wither you!", FALSE, this, NULL, victim, TO_VICT);
      act("Luckily, you have no limbs to wither.", FALSE, this, NULL, victim,
        TO_VICT);
      act("$n's prayer seems to have no effect on $N.", FALSE, this, NULL,
        victim, TO_NOTVICT);
    }
  ...
  if (found) {
    if (!silent) {
      act("$d refuses to let you wither another of $N's limbs!", FALSE, this,
        NULL, victim, TO_CHAR);
      act("Your attack goes for naught.", FALSE, this, NULL, victim, TO_CHAR);
      act("$n just tried to wither you again!", FALSE, this, NULL, victim,
        TO_VICT);
      act("$n's prayer seems to have no effect on $N.", FALSE, this, NULL,
        victim, TO_NOTVICT);
    }
...
int witherLimb(TBeing* caster, TBeing* victim, int level, short bKnown,
  int adv_learn) {
  char buf[512], limb[256];
  wearSlotT slot;
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), SPELL_WITHER_LIMB)) {
    ...
    sprintf(buf,
      "Suddenly the strength drains from your %s!\n\rIt's withering up into a "
      "useless twig!",
      limb);
    act(buf, FALSE, victim, NULL, NULL, TO_CHAR);
    sprintf(buf,
      "You watch in horror as $n's %s withers up into a useless twig!", limb);
    act(buf, FALSE, victim, NULL, NULL, TO_ROOM);
...
int witherLimb(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  int rc = witherLimb(caster, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), 0);
  int retCode = 0;
...
int witherLimb(TBeing* caster, TBeing* victim) {
  ...
  int ret =
    witherLimb(caster, victim, level, caster->getSkillValue(SPELL_WITHER_LIMB),
      caster->getAdvLearning(SPELL_WITHER_LIMB));
  int retCode = 0;
...
bool TBeing::canParalyzeLimb(TBeing* victim, silentTypeT silent) {
  if (victim->isUndead()) {
    if (!silent) {
      act("Your attempt to paralyze $N is futile!", FALSE, this, NULL, victim,
        TO_ROOM);
  ...
  if (!rc) {
    if (!silent) {
      act("$N has no limbs you can paralyze!", FALSE, this, NULL, victim,
        TO_CHAR);
      act("Your attack goes for naught.", FALSE, this, NULL, victim, TO_CHAR);
      act("$n just tried to paralyze you!", FALSE, this, NULL, victim, TO_VICT);
      act("Luckily, you have no limbs to paralyze.", FALSE, this, NULL, victim,
        TO_VICT);
      act("$n's prayer seems to have no effect on $N.", FALSE, this, NULL,
        victim, TO_NOTVICT);
    }
  ...
  if (IS_SET(rc, 1)) {
    if (!silent) {
      act("$d refuses to let you paralyze another of $N's limbs!", FALSE, this,
        NULL, victim, TO_CHAR);
      act("Your attack goes for naught.", FALSE, this, NULL, victim, TO_CHAR);
      act("$n just tried to paralyze you again!", FALSE, this, NULL, victim,
        TO_VICT);
      act("$n's prayer seems to have no effect on $N.", FALSE, this, NULL,
        victim, TO_NOTVICT);
    }
...
int paralyzeLimb(TBeing* caster, TBeing* victim, int level, short bKnown,
  int adv_learn) {
  char buf[1024], limb[256];
  wearSlotT slot;
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), SPELL_PARALYZE_LIMB)) {
    ...
    victim->addToLimbFlags(slot, PART_PARALYZED);
    sprintf(buf,
      "Suddenly the feeling disappears from your %s!\n\rIt's been paralyzed! "
      "You can't use your %s!",
      limb, limb);
    act(buf, FALSE, victim, NULL, NULL, TO_CHAR);
    ...
    if (!::number(0, 2)) {
      ...
      int rc =
        injureLimbs(caster, victim, SPELL_PARALYZE_LIMB, level, adv_learn);
      if (IS_SET_DELETE(rc, DELETE_VICT))
        return SPELL_FAIL | VICTIM_DEAD;
      return SPELL_FAIL;
    }
...
int paralyzeLimb(TBeing* caster, TBeing* victim, TMagicItem* obj) {
  ...
  int ret = paralyzeLimb(caster, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), 0);
  int retCode = 0;
  if (ret == SPELL_SUCCESS || ret == SPELL_FAIL) {
    if (!victim->isPc()) {
...
int paralyzeLimb(TBeing* caster, TBeing* v) {
  ...
  ret =
    paralyzeLimb(caster, v, level, caster->getSkillValue(SPELL_PARALYZE_LIMB),
      caster->getAdvLearning(SPELL_PARALYZE_LIMB));
  if (ret == SPELL_SUCCESS || ret == SPELL_FAIL) {
    if (!v->isPc()) {
...
int numb(TBeing* caster, TBeing* victim, int level, short bKnown,
  ...
  if (checkForTorment(victim)) {
    act("$d refuses to torment $N any further.", false, caster, 0, victim,
      TO_CHAR);
  ...
  if (!ok) {
    act("$N doesn't have a limb left to numb!", FALSE, caster, NULL, victim,
      TO_CHAR);
    ...
    act("Luckily, you have no limbs to numb.", FALSE, caster, NULL, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, NULL,
      victim, TO_NOTVICT);
    return SPELL_FAIL;
  ...
  if (found) {
    act("$d refuses to numb another of $N's limbs!", FALSE, caster, NULL,
      victim, TO_CHAR);
    act("Your attack goes for naught.", FALSE, caster, NULL, victim, TO_CHAR);
    act("$n just tried to numb you again!", FALSE, caster, NULL, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, NULL,
      victim, TO_NOTVICT);
    return SPELL_FAIL;
...
int numb(TBeing* caster, TBeing* victim, TMagicItem* obj, spellNumT spell) {
  ...
  int rc = numb(caster, victim, obj->getMagicLevel(),
    obj->getMagicLearnedness(), spell, 0);
  if (rc == SPELL_SUCCESS || rc == SPELL_FAIL) {
    if (!victim->isPc()) {
      dynamic_cast<TMonster*>(victim)->addHated(caster);
...
int numb(TBeing* caster, TBeing* victim) {
  ...
  int level = caster->getSkillLevel(spell);
  int ret = numb(caster, victim, level, caster->getSkillValue(spell), spell,
    caster->getAdvLearning(spell));
  if (ret == SPELL_SUCCESS || ret == SPELL_FAIL) {
    if (!victim->isPc()) {
      dynamic_cast<TMonster*>(victim)->addHated(caster);
...
int disease(TBeing* caster, TBeing* victim, int level, short bKnown) {
  ...
  if (victim->isImmune(IMMUNE_DISEASE, WEAR_BODY)) {
    act("$N shakes off the effects as if immune.", FALSE, caster, 0, victim,
      TO_CHAR);
    act("$n just tried to burden you with disease!", FALSE, caster, 0, victim,
      TO_VICT);
    act("You shake off the effects with ease.", FALSE, caster, 0, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, 0, victim,
      TO_NOTVICT);
    return SPELL_FAIL;
  ...
  if (caster->bSuccess(bKnown, caster->getPerc(), SPELL_DISEASE)) {
    ...
    if (!genericDisease(caster, victim, level)) {
      act("$d refuses to further disease $N.", FALSE, caster, NULL, victim,
        TO_CHAR);
      act("$d refuses $n's request to further disease you.", FALSE, caster,
        NULL, victim, TO_VICT);
      act("$n's prayer seems to have no effect on $N.", FALSE, caster, 0,
        victim, TO_NOTVICT);
      return SPELL_FAIL;
    } else {
      // messages handled in genericDisease() - they need to appear before the
      // disease starts
      return SPELL_SUCCESS;
    }
  } else {
...
int infect(TBeing* caster, TBeing* victim, int level, short bKnown,
  spellNumT spell) {
  char buf[512], limb[256];
  bool found = FALSE;
  wearSlotT slot;
  ...
  if (victim->isUndead()) {
    act("$N takes no notice.  An infection is the least of $S worries.", FALSE,
  ...
  if (slot < MAX_WEAR) {
    ...
    act("$d shows mercy!  Your attack goes for naught.", FALSE, caster, NULL,
      NULL, TO_CHAR);
    act("$n just tried to infect you again.", FALSE, caster, NULL, victim,
      TO_VICT);
    act("$n's prayer seems to have no effect on $N.", FALSE, caster, NULL,
      victim, TO_NOTVICT);
    return SPELL_FAIL;
    ...
    if (!victim->isLucky(caster->spellLuckModifier(spell))) {
      ...
      if (!victim->isLimbFlags(slot, PART_BLEEDING)) {
        if (victim->rawBleed(slot, duration, SILENT_YES, CHECK_IMMUNITY_YES)) {
          sprintf(buf,
            "A nasty cut opens up on your %s and bright red blood begins to "
            "spit out.",
            limb);
          act(buf, FALSE, victim, NULL, NULL, TO_CHAR);
          sprintf(buf, "The flesh on $n's %s cracks open and begins to bleed!",
            limb);
          act(buf, FALSE, victim, NULL, NULL, TO_ROOM);
        }
      }
      if (!victim->isLimbFlags(slot, PART_INFECTED)) {
        if (victim->rawInfect(slot, duration, SILENT_YES, CHECK_IMMUNITY_YES,
              level)) {
          sprintf(buf,
            "Your %s swells and begins to throb with pain.  It's "
            "infected!\n\rYou'd better get that cleaned up as soon as "
            "possible.",
            limb);
          act(buf, FALSE, victim, NULL, NULL, TO_CHAR);
          sprintf(buf,
            "$n recoils as $s %s swells with a vile infection.\n\rYou can "
            "virtually see the little green germs infesting it!",
            limb);
          act(buf, FALSE, victim, NULL, NULL, TO_ROOM);
        }
      }
    ...
    switch (critFail(caster, spell)) {
      ...
      case CRIT_F_HITSELF:
        ...
        if (caster->rawInfect(slot, duration, SILENT_YES, CHECK_IMMUNITY_YES,
          ...
          act("$n recoils in shock as $e infects $mself!", FALSE, caster, 0, 0,
            TO_ROOM);
        }
        return SPELL_CRIT_FAIL;
      case CRIT_F_NONE:
...
void infect(TBeing* caster, TBeing* victim, TMagicItem* obj, spellNumT spell) {
  ...
  act("$p invokes an injurious prayer against $n.", FALSE, victim, obj, 0,
    TO_ROOM);
  int ret = infect(caster, victim, obj->getMagicLevel(),
