...
bool shopData::isOwned() { return owned; }

unsigned int find_shop_nr(int mobvnum) {
  unsigned int shop_nr = 0;

  for (shop_nr = 0;
       (shop_nr < shop_index.size()) && (shop_index[shop_nr].keeper != mobvnum);
       shop_nr++)
    ;
...
float shopData::getProfitSell(const TObj* obj, const TBeing* ch) {
  ...
  if (obj) {
    ...
    if (sell_ratios_cache.count(vnum)) {
      ...
      // so check keywords
      for (iter = sell_matches_cache.begin(); iter != sell_matches_cache.end();
           ++iter) {
...
bool shopData::ensureCache() {
  ...
  db.query(
    "select obj_nr, profit_buy, profit_sell, max_num from shopownedratios "
    "where shop_nr=%i",
    shop_nr);
  while (db.fetchRow()) {
    buy_ratios_cache[convertTo<int>(db["obj_nr"])] =
      convertTo<float>(db["profit_buy"]);
    sell_ratios_cache[convertTo<int>(db["obj_nr"])] =
      convertTo<float>(db["profit_sell"]);
    max_ratios_cache[convertTo<int>(db["obj_nr"])] =
      convertTo<int>(db["max_num"]);
  }

  db.query(
    "select match_str, profit_buy, profit_sell, max_num from shopownedmatch "
    "where shop_nr=%i",
    shop_nr);
  while (db.fetchRow()) {
  ...
  db.query(
    "select max_num, corp_id, dividend, tax_nr, reserve_min, reserve_max from "
    "shopowned where shop_nr=%i",
    shop_nr);
  if (db.fetchRow()) {
  ...
  db.query(
    "select count(*) as count from rent where owner_type='shop' and owner=%i",
    shop_nr);
  if (db.fetchRow())
  ...
  if (db.fetchRow()) {
    ...
    db.query(
      "select ((sb.t+sbc.t)*%f) as t from (select count(*) as c, sum(talens) "
      "as t from shopownedbank where shop_nr=%i) sb, (select count(*) as c, "
      "sum(talens) as t from shopownedcorpbank where shop_nr=%i) sbc",
      reserve, shop_nr, shop_nr);
    if (db.fetchRow())
...
int shopData::getMaxNum(const TBeing* ch, const TObj* o, int defaultMax) {
  ...
  for (iter = max_matches_cache.begin(); o && iter != max_matches_cache.end();
       ++iter)
...
float shopData::getProfitBuy(int vnum, sstring name, const TBeing* ch) {
  ...
  if (isOwned()) {
    ...
    if (profit == -1) {
      // ok, shop is owned and there is no ratio set for this specific object
      // so check keywords
      for (iter = buy_matches_cache.begin(); iter != buy_matches_cache.end();
           ++iter) {
...
bool shopData::willBuy(const TObj* item) {
  ...
  if ((!is_commod && item->getValue() < 1) || item->isObjStat(ITEM_NEWBIE) ||
  ...
  if (mat_ok == TRUE && (shop_index[shop_nr].producing[0] == -1) &&
...
static int number_objects_in_list(const TObj* item, const StuffList list) {
  ...
  for (StuffIter it = list.begin(); it != list.end(); ++it) {
    ...
    if (i->number == item->number && i->getName() == item->getName() &&
...
void shopping_buy(const char* arg, TBeing* ch, TMonster* keeper, int shop_nr) {
  ...
  if (!(rent_id = convertTo<int>(argm))) {
    db.query(
      "select r.rent_id as rent_id, coalesce(rs.name, o.name) as name\
    ...
    db.query(
      "select r1.rent_id from rent r1 left outer join rent_strung rs1 on "
      "(r1.rent_id=rs1.rent_id), obj o1, rent r2 left outer join rent_strung "
      "rs2 on (r2.rent_id=rs2.rent_id), obj o2 where r1.vnum=r2.vnum and "
      "coalesce(rs1.short_desc, o1.short_desc)=coalesce(rs2.short_desc, "
      "o2.short_desc) and r1.owner_type='shop' and r1.owner=%i and "
      "o1.vnum=r1.vnum and r2.vnum=o2.vnum and r2.rent_id=%i",
      shop_nr, rent_id);
...
int TObj::buyMe(TBeing* ch, TMonster* keeper, int num, int shop_nr) {
  ...
  if (!ch->isImmortal()) {
    if ((ch->getCarriedVolume() + (num * getTotalVolume())) >
        ch->carryVolumeLimit()) {
      ch->sendTo(
        format("%s: You can't carry that much volume.\n\r") % fname(name));
      return -1;
    }
    ...
    if (compareWeights(getTotalWeight(TRUE),
      ...
      ch->sendTo(
        format("%s: You can't carry that much weight.\n\r") % fname(name));
      return -1;
    }
  }
  ...
  if (num > tmp) {
    keeper->doTell(ch->name,
      format("I don't have %d of that item. Here %s the %d I do have.") % num %
        ((tmp > 1) ? "are" : "is") % tmp);
  } else
    tmp = num;
  ...
  //  ch->sendTo(format("You manage to swindle the shopkeeper into a %i%s
  //  discount.\n\r") % (int)(swindle*100) % "%");
  keeper->doTell(ch->name,
    format(shop_index[shop_nr].message_buy) % (cost * count));

  ch->sendTo(COLOR_OBJECTS,
    format("You now have %s (*%d).\n\r") % sstring(getName()).uncap() % count);
  if (count == 1)
...
bool will_not_buy(TBeing* ch, TMonster* keeper, TObj* temp1, int shop_nr) {
  ...
  if (temp1->objectSell(ch, keeper)) {
    if (ch->isImmortal())
      keeper->doTell(ch->getName(),
        "Since you're immortal, I'll make an exception.");
    else
      return TRUE;
  }
  ...
  if (!temp1->stuff.empty()) {
    keeper->doTell(ch->getName(),
      "Sorry, I don't buy items that contain other items.");
    return TRUE;
  }
  ...
  if (shop_index[shop_nr].isOwned() && temp1->isObjStat(ITEM_NORENT)) {
    keeper->doTell(ch->getName(),
      "This shop is privately owned and we don't purchase non-rentable items.");
    return TRUE;
  }
  ...
  if (shop_index[shop_nr].getInventoryCount() >= (int)MAX_SHOP_INVENTORY) {
    keeper->doTell(ch->getName(),
      "My inventory is full, I can't buy anything!");
    return TRUE;
  }
...
bool TObj::sellMeCheck(TBeing* ch, TMonster* keeper, int,
  ...
  for (shop_nr = 0; (shop_nr < shop_index.size()) &&
                    (shop_index[shop_nr].keeper != (keeper)->number);
       shop_nr++)
    ;
...
void generic_num_sell(TBeing* ch, TMonster* keeper, TObj* obj, int shop_nr,
  int num) {
...
int TObj::sellMe(TBeing* ch, TMonster* keeper, int shop_nr, int num = 1) {
  ...
  keeper->doTell(ch->getName(),
    format(shop_index[shop_nr].message_sell) % cost);
...
int TThing::sellHidenSkin(TBeing*, TMonster*, int, TThing*) { return FALSE; }
...
// returns DELETE_THIS, VICT (ch), ITEM(sub)
int TSpellBag::componentSell(TBeing* ch, TMonster* keeper, int shop_nr,
  TThing*) {
  TThing* t;
  int rc;
...
tObjectManipT ObjectManipType(sstring tStString, sstring& tStBuffer,
  itemTypeT& tItem) {
  if (tStString.empty()) {
    return OBJMAN_NULL;
    tStBuffer = "";
  ...
  if (is_abbrev(tStType, "all")) {
    ...
    for (itemTypeT tItemType = MIN_OBJ_TYPE; tItemType < MAX_OBJ_TYPES;
         tItemType++)
...
bool TObj::fitsSellType(tObjectManipT tObjectManip, TBeing* ch,
...
int shopping_sell(const char* tString, TBeing* ch, TMonster* tKeeper,
  int shop_nr) {
        ...
        if (temp2) {
          if (num > 1) {
            ...
            if (soldComps.find(temp2->objVnum()) == soldComps.end()) {
              ...
              rc = temp2->componentNumSell(ch, tKeeper, shop_nr, NULL,
                temp2->getComponentCharges());
            }
          }
        }
        if (IS_SET_DELETE(rc, DELETE_THIS)) {
...
void shopping_value(const char* arg, TBeing* ch, TMonster* keeper,
  int shop_nr) {
  char argm[MAX_INPUT_LENGTH], newarg[MAX_INPUT_LENGTH];
  sstring buf;
  TObj* temp1;
...
const sstring TObj::shopList(const TBeing* ch, const sstring& arg, int iMin,
  int iMax, int num, int shop_nr, int k, unsigned long int FitT) const {
  ...
  if (!ch->canUseEquipment(this, SILENT_YES)) {
    ...
    if (isPaired()) {
      ...
      if (compareWeights(getWeight(),
            ch->maxWieldWeight(this, HAND_TYPE_BOTH)) == -1) {
        if ((isSlashWeapon() &&
      ...
      if (compareWeights(getWeight(),
        ...
        // weight > ch-wield_weight
        if (compareWeights(getWeight(),
              ch->maxWieldWeight(this, HAND_TYPE_SEC)) == -1) {
          if (tbc && tbc->isShield())
  ...
  } else if ((slot != WEAR_NECK) && (slot != WEAR_FINGER_R) &&
             (slot != WEAR_FINGER_L)) {
  ...
  if (isWearable) {
    if (tWeapon) {
      if (isPaired() || (tbc && tbc->isShield()) ||
          compareWeights(getWeight(),
            ch->maxWieldWeight(this, HAND_TYPE_SEC) == -1))
  ...
  for (counter = 0; counter < max_trade; counter++) {
    if ((shop_index[shop_nr].type[counter] == ITEM_WORN) ||
      ...
      sprintf(buf, "%s[%2d] %-29s %s%-12s %-6d %-5s %s%s\n\r", wcolor, k + 1,
        sstring(capbuf).cap().c_str(), equipColor, equipCond, cost, buf4, buf3,
        ch->norm());
      found = TRUE;
  ...
  if (!found) {
    ...
    sprintf(buf, "%s[%2d] %-31s %s%-12s %-6d %-5s\n\r", wcolor, k + 1,
      sstring(capbuf).cap().c_str(), equipColor, equipCond, cost, buf4);
  }
...
void shopping_list(sstring argument, TBeing* ch, TMonster* keeper,
  int shop_nr) {
  TDatabase db(DB_SNEEZY);
  ...
  for (int i = 0; !argument.word(i).empty(); ++i) {
    if (is_abbrev(argument.word(i), "fit"))
      FitT |= (1 << 0);
    else if (is_abbrev(argument.word(i), "slash"))
      FitT |= (1 << 1);
    else if (is_abbrev(argument.word(i), "pierce"))
      FitT |= (1 << 2);
    else if (is_abbrev(argument.word(i), "blunt"))
      FitT |= (1 << 3);
    else if (is_abbrev(argument.word(i), "body"))
      FitT |= (1 << 4);
    else if (is_abbrev(argument.word(i), "finger"))
      FitT |= (1 << 5);
    else if (is_abbrev(argument.word(i), "wrist"))
      FitT |= (1 << 6);
    else if (is_abbrev(argument.word(i), "legs"))
      FitT |= (1 << 7);
    else if (is_abbrev(argument.word(i), "arms"))
      FitT |= (1 << 8);
    else if (is_abbrev(argument.word(i), "neck"))
      FitT |= (1 << 9);
    else if (is_abbrev(argument.word(i), "feet"))
      FitT |= (1 << 10);
    else if (is_abbrev(argument.word(i), "hands"))
      FitT |= (1 << 11);
    else if (is_abbrev(argument.word(i), "head"))
      FitT |= (1 << 12);
    else if (is_abbrev(argument.word(i), "back"))
      FitT |= (1 << 13);
    else if (is_abbrev(argument.word(i), "waist"))
      FitT |= (1 << 14);
  ...
  while (db.fetchRow()) {
    ...
    if (type == ITEM_RAW_MATERIAL) {
      ...
      price =
        TCommodity::demandCurvePrice(1, 0, mat, convertTo<int>(db["count"]));
      short_desc = format("COMMODITY: %s") % material_nums[mat].mat_name;
    }

    // modify price for structure damage
    price *= ((convertTo<float>(db["max_struct"]) <= 0)
                ? 1
                : (convertTo<float>(db["cur_struct"]) /
                    convertTo<float>(db["max_struct"])));

    if (type == ITEM_POTION)
      price = liquidInfo[(liqTypeT)convertTo<int>(db["val2"])]->price *
    ...
    if (type == ITEM_ARMOR || type == ITEM_ARMOR_WAND || type == ITEM_WORN) {
      ...
      if ((slot != WEAR_NECK) && (slot != WEAR_FINGER_R) &&
        ...
        if (volume > (int)(perc / 0.85) || volume < (int)(perc / 1.15))
          fit = false;
      }
    }
    ...
    // check restrictions
    if (((FitT & (1 << 0)) && !fit) || ((FitT & (1 << 1)) && !isSlash) ||
        ((FitT & (1 << 2)) && !isPierce) || ((FitT & (1 << 3)) && !isBlunt) ||
        ((FitT & (1 << 4)) && slot != WEAR_BODY) ||
        ((FitT & (1 << 5)) && slot != WEAR_FINGER_L && slot != WEAR_FINGER_R) ||
        ((FitT & (1 << 6)) && slot != WEAR_WRIST_L && slot != WEAR_WRIST_R) ||
        ((FitT & (1 << 7)) && slot != WEAR_LEG_L && slot != WEAR_LEG_R) ||
        ((FitT & (1 << 8)) && slot != WEAR_ARM_L && slot != WEAR_ARM_R) ||
        ((FitT & (1 << 9)) && slot != WEAR_NECK) ||
        ((FitT & (1 << 10)) && slot != WEAR_FOOT_L && slot != WEAR_FOOT_R) ||
        ((FitT & (1 << 11)) && slot != WEAR_HAND_L && slot != WEAR_HAND_R) ||
    ...
    if (type == ITEM_RAW_MATERIAL) {
      buf += format("[%8i] %s COMMODITY  [%6i] %7.3f\n\r") %
             convertTo<int>(db["rent_id"]) % list_string(short_desc, 40) %
             convertTo<int>(db["count"]) % (max((float)1.0, price));
      ...
      if (mapFileToSpellnum(convertTo<int>(db["val2"])) > -1) {
        if (ch->doesKnowSkill(mapFileToSpellnum(convertTo<int>(db["val2"]))))
          spell =
            discArray[mapFileToSpellnum(convertTo<int>(db["val2"]))]->name;
      }
      ...
      buf += format("[%8i] %s %s [%6i] %7i\n\r") %
             ...
             list_string(equip_cond(convertTo<int>(db["cur_struct"]),
               ...
               10) %
             convertTo<int>(db["count"]) % (int)(max((float)1.0, price));
    }
  }
...
void TMonster::autoCreateShop(int shop_nr) {
  ...
  if (!stuff
         .empty())  // just can't see the shopkeepers inventory so lists nada?
    return;

  vlogf(LOG_MISC,
    format("Creating a new shopfile for %s (shop #%d)") % getName() % shop_nr);
  ...
  act("$n returns laden with new goodies ready to sell.", 0, this, 0, 0,
    TO_ROOM);
...
static bool shopping_look(const char* arg, TBeing* ch, TMonster* keeper,
  int shop_nr) {
  const char* tmp_desc;
  TObj* temp1;
  int rent_id;
  ...
  if (!(rent_id = convertTo<int>(arg))) {
    sstring query =
      "select r.rent_id from obj o, rent r left outer join rent_strung rs on "
      "(rs.rent_id=r.rent_id) where r.vnum=o.vnum and r.owner_type='shop' and "
      "r.owner=%i ";
    ...
    for (int i = 0; !arg_words.word(i).empty(); ++i) {
      db.escape_string(buf, arg_words.word(i).c_str(),
        arg_words.word(i).length());

      query += format(
                 "and ((rs.name is not null and rs.name like '%s%s%s') or "
                 "(o.name like '%s%s%s'))") %
               "%%" % buf % "%%" % "%%" % buf % "%%";
    }
  ...
  tmp_desc = NULL;
  if (temp1 && temp1->ex_description &&
      (tmp_desc =
          temp1->ex_description->findExtraDesc(fname(temp1->name).c_str()))) {
...
// if we process the command, return TRUE.
// if they are looking at something else (shopkeeper, etc) return FALSE
static bool shopping_evaluate(const char* arg, TBeing* ch, TMonster* keeper,
  int shop_nr) {
  char newarg[100];
  int num;
  TObj* temp1;
...
int shopping_produce(TMonster* keeper) {
  ...
  for (iter = shop_index[shop_nr].producing.begin();
    ...
    if (!(o = read_object(*iter, REAL))) {
      vlogf(LOG_BUG,
        format("Shopkeeper %d couldn't load produced item.") % shop_nr);
      db.query("commit");
      return FALSE;
    }
    ...
    tsba.journalize(keeper->getName(), o->getName(), TX_BUYING_SERVICE, cost, 0,
      0, 0);
...
int shop_keeper(TBeing* ch, cmdTypeT cmd, const char* arg, TMonster* myself,
  TObj* o) {
  int rc;
  unsigned int shop_nr;
  ...
  if (cmd == CMD_GENERIC_PULSE) {
    ...
    for (StuffIter it = myself->roomp->stuff.begin();
      ...
      if ((tbt = dynamic_cast<TBeing*>(t)) && tbt->getTimer() > 1 &&
          !tbt->isImmortal()) {
  ...
  if (cmd == CMD_GENERIC_INIT) {
    if (!myself->isUnique()) {
      vlogf(LOG_BUG,
        format("Warning!  %s attempted to be loaded, when not unique.") %
          myself->getName());
      return TRUE;
    }
    return FALSE;
  ...
  // -Cept they don't Enter shops, simply prevent them from dropping like
  // before.
  ...
  if ((cmd == CMD_DROP) && (ch->in_room == shop_index[shop_nr].in_room)) {
    ...
    act("$N tells you, 'HEY!  Don't clutter up my shop'.", FALSE, ch, 0, myself,
      TO_CHAR);
    return TRUE;
  }
...
void shoplog(int shop_nr, TBeing* ch, TMonster* keeper, const sstring& name,
  int cost, const sstring& action) {
  int value = 0, count = 0;
  TThing* tt = NULL;
  TObj* o;

  for (StuffIter it = keeper->stuff.begin();
       it != keeper->stuff.end() && (tt = *it); ++it) {
    ++count;
  ...
  //  db.query("insert into shoplog values (%i, '%s', '%s', '%s', %i, %i, %i,
...
void bootTheShops() {
  ...
  producing_db.query(
    "select shop_nr, producing from shopproducing order by shop_nr");
  ...
  material_db.query(
    "select shop_nr, mat_type from shopmaterial order by shop_nr");
  material_db.fetchRow();

  /****** owned ******/
  TDatabase owned_db(DB_SNEEZY);
  owned_db.query(
    "select shop_nr, profit_buy, profit_sell, no_such_item1, no_such_item2, "
    "do_not_buy, missing_cash1, missing_cash2, message_buy, message_sell from "
    "shopowned order by shop_nr");
  owned_db.fetchRow();

  /****** is owned ******/
  TDatabase isowned_db(DB_SNEEZY);
  isowned_db.query(
    "select distinct shop_nr from shopowned order by shop_nr asc");
  isowned_db.fetchRow();

  TDatabase db(DB_SNEEZY);

  db.query(
    "select shop_nr, no_such_item1, no_such_item2, do_not_buy, missing_cash1, "
    "missing_cash2, message_buy, message_sell, temper1, temper2, keeper, "
    "flags, in_room, open1, close1, open2, close2, profit_buy, profit_sell "
    "from shop order by shop_nr");
  ...
  while (db.fetchRow()) {
    ...
    while (!producing_db["shop_nr"].empty() &&
      ...
      sd.producing.push_back(
        real_object(convertTo<int>(producing_db["producing"])));
      producing_db.fetchRow();
    }
  ...
  db.query(
    "select shop_nr from shopowned where tax_nr is null and corp_id not in "
    "(21, 27, 29, 28)");
  ...
  db.query(
    "select shop_nr from shop where shop_nr not in (select shop_nr from "
    "shopowned)");
...
bool safe_to_save_shop_stuff(TMonster* ch) {
  if (mob_index[ch->getMobIndex()].getNumber() < 1) {
    vlogf(LOG_BUG, format("Shopkeeper #%d got safe_to_save_shop_stuff called "
                          "when none in world!") %
                     mob_index[ch->getMobIndex()].virt);
    ch->doSay(
      "I'm not functioning properly.  Tell a god to check the logs, case 1.");
    return FALSE;
  }
  if (mob_index[ch->getMobIndex()].getNumber() > 1) {
    vlogf(LOG_BUG,
      format(
        "More than one shopkeeper #%d in world.  Now the shop won't work!") %
        mob_index[ch->getMobIndex()].virt);
    ch->doSay(
      "I'm not functioning properly.  Tell a god to check the logs, case 2.");
    return FALSE;
  }
  return TRUE;
}
...
/*void processShopFile(const char *cFname)
{
  ...
  if (!(fp = fopen(fileName, "r"))) {
    vlogf(LOG_BUG, format("  Error opening the shop file for shop #%s") %
cFname); return;
  }
  ...
  if (!noteLimitedItems(fp, fileName, ucVersion, FALSE))
    vlogf(LOG_BUG, format("  Unable to count limited items in file  %s") %
fileName); fclose(fp);
}
...
int TObj::adjPrice() const {
  int value =
    ((getMaxStructPoints() <= 0)
        ? getValue()
        : (int)(getValue() * getStructPoints() / getMaxStructPoints()));
...
void factoryProduction(int shop_nr) {
  ...
  db_vnum.query(
    "select fp.vnum as vnum, count(r.vnum) as count from factoryproducing fp "
    "left outer join rent r on (fp.vnum=r.vnum and r.owner_type='shop' and "
    "r.owner=fp.shop_nr) where fp.shop_nr=%i group by fp.vnum",
    shop_nr);
  ...
  while (db_vnum.fetchRow()) {
    ...
    db.query(
      "select fb.supplyamt as required, fs.supplyamt as avail, fs.supplyname "
      "as name from factoryblueprint fb, factorysupplies fs where fb.vnum=%i "
      "and fs.shop_nr=%i and fs.supplytype=fb.supplytype",
      vnum, shop_nr);
    ...
    shoplog(SBA_SHOP_NR, keeper, sba, obj->getName(), obj->productionPrice(),
      "producing");
    ...
    tso.journalize_credit(100, keeper->getName(), obj->getName(),
      obj->productionPrice());
    ...
    // log the sale
    shoplog(shop_nr, keeper, keeper, obj->getName(), -(obj->productionPrice()),
      "factory production");
    ...
    // save avail amt
    db.query(
      "update factorysupplies, factoryblueprint set "
      "factorysupplies.supplyamt=factorysupplies.supplyamt-factoryblueprint."
      "supplyamt where factorysupplies.shop_nr=%i and factoryblueprint.vnum=%i "
      "and factorysupplies.supplytype=factoryblueprint.supplytype",
      shop_nr, vnum);
  }
}
