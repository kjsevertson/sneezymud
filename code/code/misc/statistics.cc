...
int init_game_stats(void) {
  ...
  // stats.global_lp_target = stats.equip;  // When we want to change the load
  // potential, this is what we change. stats.global_lp_target_changerate = 0.0;
  // // This is how fast the load potential changes.
  ...
  if (!(fp = fopen(Path::MUTABLE_STATS_FILE, "r"))) {
    ...
    for (i = 0; i < MAX_IMMORT; i++) {
      if (
        fscanf(fp,
          ...
          &gold_statistics[GOLD_INCOME][i], &gold_statistics[GOLD_SHOP][i],
          &gold_statistics[GOLD_REPAIR][i], &gold_statistics[GOLD_COMM][i],
          &gold_statistics[GOLD_HOSPITAL][i], &gold_statistics[GOLD_GAMBLE][i],
          &gold_statistics[GOLD_RENT][i], &gold_statistics[GOLD_TITHE][i],
      ...
      if (
        fscanf(fp,
          " %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld\n",
          &gold_positive[GOLD_INCOME][i], &gold_positive[GOLD_SHOP][i],
          &gold_positive[GOLD_REPAIR][i], &gold_positive[GOLD_COMM][i],
          &gold_positive[GOLD_HOSPITAL][i], &gold_positive[GOLD_GAMBLE][i],
          &gold_positive[GOLD_RENT][i], &gold_positive[GOLD_TITHE][i],
          ...
          &gold_positive[GOLD_SHOP_WEAPON][i], &gold_positive[GOLD_SHOP_PET][i],
    ...
    if (fscanf(fp, " %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n",
          &gold_modifier[GOLD_INCOME], &gold_modifier[GOLD_SHOP],
          &gold_modifier[GOLD_REPAIR], &gold_modifier[GOLD_COMM],
          &gold_modifier[GOLD_HOSPITAL], &gold_modifier[GOLD_GAMBLE],
          &gold_modifier[GOLD_RENT], &gold_modifier[GOLD_TITHE],
          &gold_modifier[GOLD_SHOP_FOOD], &gold_modifier[GOLD_SHOP_COMPONENTS],
          &gold_modifier[GOLD_SHOP_SYMBOL], &gold_modifier[GOLD_SHOP_ARMOR],
          &gold_modifier[GOLD_SHOP_WEAPON], &gold_modifier[GOLD_SHOP_PET],
    ...
    for (i = 0; i < 50; i++) {
      for (j = 0; j < MAX_CLASSES; j++) {
        if (fscanf(fp, "%d %ld ", &stats.levels[j][i],
              &stats.time_levels[j][i]) != 2) {
          vlogf(LOG_BUG,
            format("Bad level info, class %d, lev %d") % j % (i + 1));
        }
      }
    }
  ...
  if ((fp = fopen(Path::MUTABLE_STATS_FILE, "w+")) != NULL) {
    ...
    for (i = 0; i < MAX_IMMORT; i++) {
      fprintf(fp,
        ...
        gold_statistics[GOLD_SHOP_SYMBOL][i],
        gold_statistics[GOLD_SHOP_ARMOR][i],
        gold_statistics[GOLD_SHOP_WEAPON][i], gold_statistics[GOLD_SHOP_PET][i],
        gold_statistics[GOLD_SHOP_RESPONSES][i], gold_statistics[GOLD_DUMP][i]);
      fprintf(fp,
        "%ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld\n",
        gold_positive[GOLD_INCOME][i], gold_positive[GOLD_SHOP][i],
        gold_positive[GOLD_REPAIR][i], gold_positive[GOLD_COMM][i],
        ...
        gold_positive[GOLD_SHOP_FOOD][i],
        gold_positive[GOLD_SHOP_COMPONENTS][i],
        gold_positive[GOLD_SHOP_SYMBOL][i], gold_positive[GOLD_SHOP_ARMOR][i],
        gold_positive[GOLD_SHOP_WEAPON][i], gold_positive[GOLD_SHOP_PET][i],
        gold_positive[GOLD_SHOP_RESPONSES][i], gold_positive[GOLD_DUMP][i]);
    }
    fprintf(fp,
      "%.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f "
      "%.2f %.2f\n",
      gold_modifier[GOLD_INCOME].getVal(), gold_modifier[GOLD_SHOP].getVal(),
      gold_modifier[GOLD_REPAIR].getVal(), gold_modifier[GOLD_COMM].getVal(),
      gold_modifier[GOLD_HOSPITAL].getVal(),
      gold_modifier[GOLD_GAMBLE].getVal(), gold_modifier[GOLD_RENT].getVal(),
...
int secs_to_level(int lev) {
  ...
  int regen_per_tick;
  regen_per_tick =
    (int)(stats.hit_gained_attempts == 0
            ? 1
...
void TBeing::doGamestats(const sstring& arg) {
  ...
  if (buf.empty()) {
    sendTo(
      "Syntax: gamestats <attributes | combat | equipment | levels | "
      "trivia>\n\r");
    return;
    ...
    buf = format("\tMob damage taken    : %ld   (%5.2f%c)\n\r") %
          stats.damage[MOB_STAT] %
          (tot_dam ? (100.0 * stats.damage[MOB_STAT] / tot_dam) : 0) % '%';
    str += buf;
    buf = format("\tPC  damage taken    : %ld   (%5.2f%c)\n\r") %
          stats.damage[PC_STAT] %
          (tot_dam ? (100.0 * stats.damage[PC_STAT] / tot_dam) : 0) % '%';
    str += buf;
    ...
    buf = format("\tMob combat damage taken    : %ld   (%5.2f%c)\n\r") %
          stats.combat_damage[MOB_STAT] %
          (tot_dam ? (100.0 * stats.combat_damage[MOB_STAT] / tot_dam) : 0) %
          '%';
    str += buf;
    buf = format("\tPC combat damage taken     : %ld   (%5.2f%c)\n\r") %
          stats.combat_damage[PC_STAT] %
          (tot_dam ? (100.0 * stats.combat_damage[PC_STAT] / tot_dam) : 0) %
          '%';
    str += buf;
    ...
    buf = format(
            "\tMob AC absorb       : %ld   (%5.2f%c of all abs) (%5.2f%c of "
            "mob ComDam)\n\r") %
          stats.ac_absorb[MOB_STAT] %
          (tot_ac ? stats.ac_absorb[MOB_STAT] * 100.0 / tot_ac : 0) % '%' %
          (mob_dam ? stats.ac_absorb[MOB_STAT] * 100.0 / mob_dam : 0) % '%';
    str += buf;

    int pc_dam = stats.ac_absorb[PC_STAT] + stats.combat_damage[PC_STAT];
    buf = format(
            "\tPC AC absorb        : %ld   (%5.2f%c of all abs) (%5.2f%c of PC "
            "ComDam)\n\r") %
          stats.ac_absorb[PC_STAT] %
          (tot_ac ? stats.ac_absorb[PC_STAT] * 100.0 / tot_ac : 0) % '%' %
    ...
    buf = format("Total Combat hits         : %ld  (%5.2f%c)\n\r") % tot_hits %
          (tot_blows == 0 ? 0.0 : (100.0 * tot_hits / tot_blows)) % '%';
    str += buf;
    buf =
      format(
        "\tMob combat hits            : %ld   (%5.2f%c)  (%5.2f%c hit "
        "rate)\n\r") %
      stats.combat_hits[MOB_STAT] %
      (tot_hits ? (100.0 * stats.combat_hits[MOB_STAT] / tot_hits) : 0) % '%' %
      (stats.combat_blows[MOB_STAT]
          ? stats.combat_hits[MOB_STAT] * 100.0 / stats.combat_blows[MOB_STAT]
          : 0) %
      '%';
    str += buf;
    buf = format(
            "\tPC combat hits             : %ld   (%5.2f%c)  (%5.2f%c hit "
            "rate)\n\r") %
          stats.combat_hits[PC_STAT] %
          (tot_hits ? (100.0 * stats.combat_hits[PC_STAT] / tot_hits) : 0) %
          '%' %
          (stats.combat_blows[PC_STAT]
              ? stats.combat_hits[PC_STAT] * 100.0 / stats.combat_blows[PC_STAT]
              : 0) %
          '%';
    str += buf;
    ...
    str += buf;
    buf =
      format("\tMob combat level           : %5.2f\n\r") %
      (stats.combat_blows[MOB_STAT] ? ((double)stats.combat_level[MOB_STAT] /
                                        (double)stats.combat_blows[MOB_STAT])
                                    : 0.0);
    str += buf;
    buf = format("\tPC combat level            : %5.2f\n\r") %
          (stats.combat_blows[PC_STAT] ? ((double)stats.combat_level[PC_STAT] /
                                           (double)stats.combat_blows[PC_STAT])
                                       : 0.0);
    str += buf;
    ...
    str += buf;
    buf =
      format("\tMob avg. combat dam.       : %5.2f\n\r") %
      (stats.combat_hits[MOB_STAT] ? ((double)stats.combat_damage[MOB_STAT] /
                                       (double)stats.combat_hits[MOB_STAT])
                                   : 0.0);
    str += buf;
    buf = format("\tPC avg. combat dam.        : %5.2f\n\r") %
          (stats.combat_hits[PC_STAT] ? ((double)stats.combat_damage[PC_STAT] /
                                          (double)stats.combat_hits[PC_STAT])
                                      : 0.0);
    str += buf;

    buf = format("Total crit-success checks : %ld  (%5.2f%c of hits)\n\r") %
          stats.combat_crit_suc %
          (tot_hits == 0 ? 0.0 : (100.0 * stats.combat_crit_suc / tot_hits)) %
          '%';
    str += buf;
    buf = format("Total crit-fail checks    : %ld  (%5.2f%c of misses)\n\r") %
          stats.combat_crit_fail %
          ((tot_blows - tot_hits) == 0
              ? 0.0
              : (100.0 * stats.combat_crit_fail / (tot_blows - tot_hits))) %
          '%';
    str += buf;
    buf =
      format("Total crit-success passes : %ld  (%5.2f%c of hits)\n\r") %
      stats.combat_crit_suc_pass %
      (tot_hits == 0 ? 0.0 : (100.0 * stats.combat_crit_suc_pass / tot_hits)) %
      '%';
    str += buf;
    buf =
      format("Total crit-fail passes    : %ld  (%5.2f%c of misses)\n\r") %
    ...
    if ((lev >= 0) && (lev < GameStats::MAX_DEATH_LEVEL)) {
      ...
      if (lev >= 1 && lev <= 50) {
        ...
        sendTo(format("Desired leveling time: %s\n\r") % secsToString(factor));

        for (i = 0; i < MAX_CLASSES; i++) {
          time_t factor = 0;
          if (stats.levels[i][lev - 1])
            factor = SECS_PER_REAL_MIN * stats.time_levels[i][lev - 1] /
                     stats.levels[i][lev - 1];
...
int getNetGoldGlobal() {
  ...
  for (i = 0; i < MAX_MORT; i++)
    net_gold +=
      gold_statistics[GOLD_INCOME][i] + gold_statistics[GOLD_COMM][i] +
      gold_statistics[GOLD_RENT][i] + gold_statistics[GOLD_REPAIR][i] +
      gold_statistics[GOLD_HOSPITAL][i] + gold_statistics[GOLD_GAMBLE][i] +
      gold_statistics[GOLD_TITHE][i] + gold_statistics[GOLD_DUMP][i] +
      gold_statistics[GOLD_SHOP_FOOD][i] +
      gold_statistics[GOLD_SHOP_COMPONENTS][i] +
      gold_statistics[GOLD_SHOP_SYMBOL][i] +
      gold_statistics[GOLD_SHOP_ARMOR][i] +
      gold_statistics[GOLD_SHOP_WEAPON][i] +
      gold_statistics[GOLD_SHOP_RESPONSES][i] +
      gold_statistics[GOLD_SHOP_PET][i] + gold_statistics[GOLD_SHOP][i];
...
unsigned int getPosGoldGlobal() {
  ...
  for (i = 0; i < MAX_MORT; i++)
    pos_gold +=
      gold_positive[GOLD_INCOME][i] + gold_positive[GOLD_COMM][i] +
      gold_positive[GOLD_RENT][i] + gold_positive[GOLD_REPAIR][i] +
      gold_positive[GOLD_HOSPITAL][i] + gold_positive[GOLD_GAMBLE][i] +
      gold_positive[GOLD_DUMP][i] + gold_positive[GOLD_TITHE][i] +
      gold_positive[GOLD_SHOP_FOOD][i] +
      gold_positive[GOLD_SHOP_COMPONENTS][i] +
      gold_positive[GOLD_SHOP_SYMBOL][i] + gold_positive[GOLD_SHOP_ARMOR][i] +
      gold_positive[GOLD_SHOP_WEAPON][i] + gold_positive[GOLD_SHOP_PET][i] +
      gold_positive[GOLD_SHOP_RESPONSES][i] + gold_positive[GOLD_SHOP][i];
...
int getNetGoldBudget() {
  ...
  for (i = 0; i < MAX_MORT; i++)
    net_gold +=
      gold_statistics[GOLD_INCOME][i] + gold_statistics[GOLD_COMM][i] +
      gold_statistics[GOLD_REPAIR][i] + gold_statistics[GOLD_HOSPITAL][i] +
      gold_statistics[GOLD_GAMBLE][i] + gold_statistics[GOLD_TITHE][i] +
      gold_statistics[GOLD_DUMP][i] + gold_statistics[GOLD_SHOP_FOOD][i] +
      gold_statistics[GOLD_SHOP_COMPONENTS][i] +
      gold_statistics[GOLD_SHOP_SYMBOL][i] +
      gold_statistics[GOLD_SHOP_ARMOR][i] +
      gold_statistics[GOLD_SHOP_WEAPON][i] +
      gold_statistics[GOLD_SHOP_RESPONSES][i] + gold_statistics[GOLD_SHOP][i];
...
unsigned int getPosGoldBudget() {
  ...
  for (i = 0; i < MAX_MORT; i++)
    pos_gold +=
      gold_positive[GOLD_INCOME][i] + gold_positive[GOLD_COMM][i] +
      gold_positive[GOLD_REPAIR][i] + gold_positive[GOLD_HOSPITAL][i] +
      gold_positive[GOLD_GAMBLE][i] + gold_positive[GOLD_DUMP][i] +
      gold_positive[GOLD_TITHE][i] + gold_positive[GOLD_SHOP_FOOD][i] +
      gold_positive[GOLD_SHOP_COMPONENTS][i] +
      gold_positive[GOLD_SHOP_SYMBOL][i] + gold_positive[GOLD_SHOP_ARMOR][i] +
      gold_positive[GOLD_SHOP_WEAPON][i] +
      gold_positive[GOLD_SHOP_RESPONSES][i] + gold_positive[GOLD_SHOP][i];
...
void procCheckGoldStats::run(const TPulse&) const {
  ...
  if (net_gold_all_shops > 0) {
    ...
    //    vlogf(LOG_BUG, format("ECONOMY: shop modifier lowered. %d %u %.2f") %
    //    net_gold_all_shops % pos_gold_all_shops %
    ...
    gold_modifier[GOLD_SHOP] += 0.01;
    //    vlogf(LOG_BUG, format("ECONOMY: shop modifier raised. %d %u %.2f") %
    //    net_gold_all_shops % pos_gold_all_shops %
    //    gold_modifier[GOLD_SHOP].getVal());
    should_reset = true;
  }
  ...
  if ((unsigned int)net_gold_budget <
      ((target_income - 0.03) * pos_gold_budget)) {
    // players losing money
    gold_modifier[GOLD_INCOME] += 0.01;
    //    vlogf(LOG_BUG, format("ECONOMY: income modifier raised. %d %u %.2f") %
    //    net_gold_budget % pos_gold_budget %
    //    gold_modifier[GOLD_INCOME].getVal());
    should_reset = true;
  } else if ((unsigned int)net_gold_budget >
             ((target_income + 0.03) * pos_gold_budget)) {
    // players making too much
    gold_modifier[GOLD_INCOME] -= 0.01;
    //    vlogf(LOG_BUG, format("ECONOMY: income modifier lowered. %d %u %.2f")
    //    %  net_gold_budget % pos_gold_budget %
    //    gold_modifier[GOLD_INCOME].getVal());
    should_reset = true;
  }
