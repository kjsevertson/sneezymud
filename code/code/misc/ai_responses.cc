...
int TMonster::modifiedDoCommand(cmdTypeT cmd, const sstring& arg, TBeing* mob,
  const resp* respo) {
  int rc = 0;
  int value;
  TObj* obj = NULL;
  ...
  sstring tStObj(""), tStSucker(""), tStArgument(arg);
  TPathFinder path;
  TThing* tThing;
  ...
  switch (cmd) {
    ...
    case CMD_RESP_UNFLAG:
      if (mob->isPlayerAction(PLR_SOLOQUEST)) {
        ...
        act("$n just removed your solo quest flag.", FALSE, this, 0, mob,
          TO_VICT);
      }
      return TRUE;
    case CMD_FLAG:
    ...
    case CMD_RESP_CHECKLOAD:
      if (mobVnum() < 0) {
        doTell(mob->getNameNOC(this),
          "I would load it, but i'm a prototype.  Sorry.");
        return FALSE;  // continue the script, even tho this is a 'dummy'
                       // trigger.
      }
      ...
      if (value <= 0 || ((rc = real_object(value)) <= 0)) {
        vlogf(LOG_MOB_RS,
          format("Problem in script (1).  Trying to load %d on %s") % value %
            getName());
        return FALSE;
      }
      if (!(obj = read_object(rc, REAL))) {
        vlogf(LOG_MOB_RS,
          format("Problem in script (2).  Trying to load %d on %s") % value %
            getName());
        return FALSE;
      }
      if (obj_index[rc].getNumber() > obj_index[rc].max_exist) {
        vlogf(LOG_MOB_RS,
          format(
            "Quest mob (%s:%d) loading item (%s:%d) when over max_exist.") %
            getName() % mobVnum() % obj->getName() % obj->objVnum());
      }
      TThing* t;
      t = NULL;
      for (StuffIter it = stuff.begin(); it != stuff.end() && (t = *it); ++it) {
        ...
        if (tob->number == obj->number) {
          ...
          if (cmd == CMD_LOAD) {
            ...
            vlogf(LOG_MOB_RS,
              format(
                "Quest mob (%s) tried loading item (%s) that he already had.") %
                getName() % obj->getName());
            delete obj;
    ...
    case CMD_RESP_LOADMOB: {
      ...
      if (value <= 0 || ((rc = real_mobile(value)) <= 0)) {
        vlogf(LOG_MOB_RS,
          format("Problem in script (3).  Trying to load %d on %s") % value %
            getName());
        return FALSE;
      }
      if (!(tMonster = read_mobile(rc, REAL))) {
        vlogf(LOG_MOB_RS,
          format("Problem in script (4).  Trying to load %d on %s") % value %
            getName());
        return FALSE;
      }
      if (mob_index[rc].getNumber() > mob_index[rc].max_exist) {
        vlogf(LOG_MOB_RS,
          format("Quest mob (%s:%d) loading mob (%s:%d) when over max_exist.") %
            getName() % mobVnum() % tMonster->getName() % tMonster->mobVnum());
      }
      if (mob_index[rc].spec == SPEC_SHOPKEEPER) {
        vlogf(LOG_MOB_RS,
          format(
            "Problem in script.  %s trying to load %d which is a shopkeeper.") %
            getName() % value);
        delete tMonster;
        return FALSE;
      }
      if (mob_index[rc].spec == SPEC_NEWBIE_EQUIPPER) {
        vlogf(LOG_MOB_RS, format("Problem in script.  %s trying to load %d "
                                 "which is a newbie helper.") %
                            getName() % value);
        delete tMonster;
        return FALSE;
      }
    ...
    case CMD_RESP_PERSONALIZE:
      ...
      if (value <= 0 || (real_object(value) < 0)) {
        vlogf(LOG_MOB_RS,
          format("Problem in script (5).  Trying to load %d on %s") % value %
            getName());
        return FALSE;
      }
    ...
    case CMD_RESP_RESIZE:
      ...
      if (value <= 0 || (real_object(value) < 0)) {
        vlogf(LOG_MOB_RS,
          format("Problem in script (6).  Trying to load %d on %s") % value %
            getName());
        return FALSE;
      }
    ...
    case CMD_RESP_TOGGLE:
      ...
      if (value <= 0 || value >= MAX_TOG_INDEX) {
        vlogf(LOG_MOB_RS,
          format("Bad argument to response (%s) command %d.  (%s)") % name %
            cmd % arg);
        return FALSE;
      }
      if (TogIndex[value].togmob != Mob::ANY &&
          TogIndex[value].togmob != mobVnum()) {
        vlogf(LOG_MOB_RS, format("Wrong mob (%s:%d) toggling toggle %d.") %
    ...
    case CMD_RESP_UNTOGGLE:
      ...
      if (value <= 0 || value >= MAX_TOG_INDEX) {
        vlogf(LOG_MOB_RS,
          format("Bad argument to response (%s) special command %d.  (%s)") %
            name % cmd % arg);
        return FALSE;
      }
      if (!mob->hasQuestBit(value)) {
        vlogf(LOG_MOB_RS,
          format("Response file untoggling an unset bit: %d on %s by %s '%s'") %
    ...
    case CMD_RESP_CHECKMAX:
      ...
      if (value <= 0 || value >= (signed)obj_index.size()) {
        vlogf(LOG_MOB_RS,
          format("Bad argument to response (%s) special command %s.  (%s)") %
            name % cmd % arg);
        return FALSE;
      }
    ...
    case CMD_RESP_LINK:
      ...
      if (buf.lower() == "roomenter") {
        ...
        if (cmd_val >= MAX_CMD_LIST) {
          vlogf(LOG_MOB_RS,
            format("Responses::readCommand(): Parse error in %s. link bad.") %
              buf);
          return RET_STOP_PARSING;
        }
      }
    ...
    case CMD_JUNK:
      ...
      if (!mob || !tThing)
        vlogf(LOG_BUG,
          format("Error in response script junk command:%s%s Mob[%s]") %
            (mob ? "" : " !mob") % (tThing ? "" : " !tThing") %
            getNameNOC(this));
    ...
    case CMD_GIVE:
      ...
      // the force drop shit is lame, so another safety check here. give SHOULD
      // return FALSE if it failed, soooo... i'm gonna throw in a little hack
      // -dash
      tStObj = tStArgument.word(0);
      tStSucker = tStArgument.word(1);
      ...
      if (!rc) {
        ...
        if (tThing && tThing->parent == this && roomp) {
          ...
          *roomp += *tThing;
          ...
          act("You drop $p", FALSE, this, tThing, NULL, TO_CHAR);
          act("$n drops $p.", FALSE, this, tThing, NULL, TO_ROOM);
        }
      }
      break;
    ...
    case CMD_RESP_CHECKZONE:
      ...
      if ((!roomp || !tRoom || tRoom->getZoneNum() != roomp->getZoneNum()) &&
          !inImperia())
        return RET_STOP_PARSING;
    ...
    case CMD_RESP_DESTINATION:
      ...
      for (rMem = resps->respMemory, lMem = resps->respMemory; rMem;
           rMem = rMem->next) {
        if (rMem->cmd == CMD_RESP_DESTINATION) {
    ...
    case CMD_RESP_SETMODE: {
      if (arg == "soloquest") {
        ...
        for (checkAffect = affected;
             checkAffect && checkAffect->type == AFFECT_COMBAT &&
             checkAffect->modifier == COMBAT_SOLO_KILL &&
             checkAffect->level == myVnum;
...
bool TMonster::checkResponsesPossible(cmdTypeT tCmd, const sstring& tSaid,
  TBeing* tBeing) {
...
int handleMobileResponse(TBeing* tBeing, cmdTypeT tCmd,
  const sstring& tString) {
  ...
  for (StuffIter it = tBeing->roomp->stuff.begin();
       it != tBeing->roomp->stuff.end() && (tThing = *it); ++it)
    if ((tMonster = dynamic_cast<TMonster*>(tThing)) && !tMonster->isPc() &&
        !tMonster->orig) {
...
int TMonster::checkResponses(TBeing* tBeing, TThing* tThing,
  const sstring& tSaid, cmdTypeT tCmd) {
  ...
  if ((tTask = task)) {
    act("You interrupt your task to deal with $N", FALSE, this, NULL, tBeing,
      TO_CHAR);
    act("$n interrupts $s task for a moment.", FALSE, this, NULL, tBeing,
      TO_ROOM);
  ...
  if ((tSpell = spelltask)) {
    act("You interrupt your spell to deal with $N", FALSE, this, NULL, tBeing,
      TO_CHAR);
    act("$n interrupts $s task for a moment.", FALSE, this, NULL, tBeing,
      TO_ROOM);
  ...
  if (tTask) {
    act("You resume your task.", FALSE, this, NULL, tThing, TO_CHAR);

    task = tTask;
  }

  if (tSpell) {
    act("You resume your spell.", FALSE, this, NULL, tThing, TO_CHAR);
  ...
  switch (cmd) {
    case CMD_RESP_RANDOM:
      if (!(arg_int = convertTo<int>(parsedArgs))) {
        vlogf(LOG_MOB_RS, "Bad argument to doRandCmd.");
        return 0;
      }
...
void responseTransaction(TBeing* speaker, TMonster* tm, int value) {
  ...
  if (keeper) {
    ...
    tso.journalize(tm->getName(), speaker->getName(), TX_BUYING_SERVICE, value,
      0, 0, 0);

    shoplog(SBA_SHOP_NR, speaker, keeper, tm->getName(), value, "giving");
...
int TMonster::checkResponsesReal(TBeing* speaker, TThing* resp_targ,
  const sstring& said, cmdTypeT trig_cmd) {
  sstring parsedArgs;
  resp* respo;
  int found = FALSE;
  int rc;
  ...
  char tString[256];
  int skip =
    0;  // command lines within brackets will be skipped if this is non-zero
      ...
      switch (trig_cmd) {
        ...
        case CMD_GIVE:
          if (!(value = convertTo<int>(respo->args))) {
            vlogf(LOG_MOB_RS,
              format("Bad arguments for %s for Give") % getName());
            break;
          }
          if (value > 0) {
            TObj* to = NULL;
        ...
        case CMD_BUY:
          ...
          if ((is_number(tString)
            ...
            if (speaker->getMoney() < value) {
              doTell(speaker->getNameNOC(this),
                "I'm afraid you don't have enough for that.");
        ...
        case CMD_GRIMACE:
          if ((isname("none", respo->args) && (resp_targ == NULL)) ||
              ...
              (isname("other", respo->args) && (resp_targ != this) &&
        ...
        default:
          vlogf(LOG_MOB_RS,
            format("Unknown command is checkResponse: %d") % trig_cmd);
          break;
      }  // end of switch
    }
  }
...
bool readToChar(std::istringstream& is, char* buf, char chr,
  bool care = false) {
  char* ptr = buf;
  int c;
...
static void sstringTranslate(char* buf) {
  ...
  while ((start_pt = strstr(buf, "#OBJCOST("))) {
    ...
    if (rc != 2) {
      vlogf(LOG_LOW,
        format("sstringTranslate(): failed parse OBJCOST (%d)") % rc);
      return;
    }
    int robj = real_object(obj_num);
    if (robj < 0) {
      vlogf(LOG_LOW, format("sstringTranslate(): bad vnum (%d)") % obj_num);
      return;
    }
    ...
    if (obj_index[robj].itemtype == ITEM_POTION) {
      ...
      if (obj) {
        if (pot) {
          ...
          vlogf(LOG_LOW,
            format(
              "Error casting object as potion in response script: %s(%d)") %
              obj->getName() % robj);
        }
        pot = NULL;
      } else {
        price = 0;
        vlogf(LOG_LOW,
          format("Error loading potion object in response script: %d") % robj);
      }
      delete obj;
      obj = NULL;
    } else {
...
resp* TMonster::readCommand(std::istringstream& is) {
  ...
  cleanInputBuffer(is);
  if ((c = is.get()) != '{') {
    vlogf(LOG_LOW, format("Responses::readCommand(): Parse error in %s. Error "
                          "after '%s'. Expected '{' but found '%c'") %
  ...
  if (!strcasecmp(cmdStr, "roomenter")) {
    ...
    if (cmd >= MAX_CMD_LIST) {
      vlogf(LOG_LOW,
        format(
          "Responses::readCommand(): Parse error in %s. Unknown command %s.") %
          responseFile % cmdStr);
      return 0;
    }
  }
  ...
  while (*buf != 0) {
    ...
    if (is_abbrev(buf, "toggle"))
      ...
      if (strstr(buf, "$N")) {
        vlogf(LOG_LOW,
          format("Found '$N' on toroom command inside response file for %s") %
            responseFile);
        delete newResp;
        newResp = NULL;
        return 0;
      ...
      if ((cmd == CMD_ECHO) || (cmd == CMD_EMOTE)) {
        vlogf(LOG_MOB_RS, format("potential bad command (%s) in %s response") %
                            buf % getName());
        delete newResp;
        return 0;
      }
...
int specificCode(TMonster* mob, TBeing* ch, int which, const resp* respo) {
  ...
  switch (which) {
    case 1:
      if (mob->mobVnum() != 3459) {
        vlogf(LOG_LOW, format("Bad mob (%s:%d) calling specificCode(%d)") %
                         mob->getName() % mob->mobVnum() % which);
        return RET_STOP_PARSING;
      }
      act(
        "A swirling mist envelops you, as you bury your head between $N's "
        "legs.",
        FALSE, ch, 0, mob, TO_CHAR);
      act(
        "A swirling mist envelops $n, as $e buries $s head between your legs.",
        FALSE, ch, 0, mob, TO_VICT);
      act(
        "A swirling mist envelops $n, as $e buries $s head between $N's legs.",
      ...
      act("$S thighs tighten around $s neck, squeezing the life out of $m.",
        FALSE, ch, 0, mob, TO_NOTVICT);
      act(
        "You struggle frantically, but $N's powerful muscles sever your head "
        "from your shoulders!",
        FALSE, ch, 0, mob, TO_CHAR);
      act(
        "$n struggles frantically, but your powerful muscles sever $s head "
        "from $s shoulders!",
        FALSE, ch, 0, mob, TO_VICT);
      act(
        "$n struggles frantically, but $N's powerful muscles sever $s head "
        "from $s shoulders!",
        FALSE, ch, 0, mob, TO_NOTVICT);
    ...
    case 2:
      ...
      if (!ch->isUnaff()) {
        if (!ch->isBrother())
          mob->doSay(
            "I'm afraid you can't become part of the Brotherhood when you "
            "belong to another faction already.");
        else
    ...
    case 3:
      ...
      if (!ch->isUnaff()) {
        if (!ch->isCult())
          mob->doSay(
            "I'm afraid you can't become part of the Sect when you belong to "
            "another faction already.");
        else
    ...
    case 4:
      ...
      if (!ch->isUnaff()) {
        if (!ch->isSnake())
          mob->doSay(
            "I'm afraid you can't become part of the Order when you belong to "
            "another faction already.");
        else
    ...
    case 5:
      ...
      if (!(tSymbol = dynamic_cast<TSymbol*>(read_object(504, VIRTUAL)))) {
        act(
          "$n looks befuddled and says, \"I'm sorry...I can not seem to find a "
          "symbol for you.\"",
        ...
        act("$n reaches into his grey smock and pulls a symbol from beneath.",
          TRUE, mob, 0, ch, TO_ROOM);
        act(
          "$n utters something barely audible and passes his hand above the "
          "symbol.",
          TRUE, mob, 0, ch, TO_ROOM);
        act("$n grins with delight as the symbol shines with power.", TRUE, mob,
          0, ch, TO_ROOM);
    ...
    case 6:
      ...
      act("$n murmurs over a mermaids scale.", FALSE, mob, NULL, NULL, TO_ROOM);
      act("$n draws upon the primordial force of the mermaid.", FALSE, mob,
        NULL, NULL, TO_ROOM);
      act("A mermaids scale begins to spark and flash.", FALSE, mob, NULL, NULL,
        TO_ROOM);
      act("A giant mass of balled lightning forms above the mermaids scale.",
        FALSE, mob, NULL, NULL, TO_ROOM);
      act("A giant ball of blue-white lightning erupts from $n's hands!", FALSE,
      ...
      for (; FDt; FDt = FDt->next) {
        ...
        act("You are enveloped by a ball of pure blue-white light!", FALSE,
          FDt->follower, NULL, NULL, TO_CHAR);
        act("A blue-white ball of lightning surrounds $n!", TRUE, FDt->follower,
          NULL, NULL, TO_ROOM);

        tAff.duration = ((Pulse::ONE_SECOND * 60) * 60) * ::number(12, 24);
        tAff.modifier2 = ::number(25, 75);
        FDt->follower->affectJoin(FDt->follower, &tAff, AVG_DUR_NO,
          AVG_EFF_YES);
      }

      act("You are enveloped by a ball of pure blue-white light!", FALSE, ch,
        NULL, NULL, TO_CHAR);
      act("A blue-white ball of lightning surrounds $n!", TRUE, ch, NULL, NULL,
        TO_ROOM);
      tAff.duration = ((Pulse::ONE_SECOND * 60) * 60) * ::number(12, 24);
      tAff.modifier2 = ::number(25, 75);
      ...
      if (ch->master) {
        ...
        act("A blue-white ball of lightning surrounds $n!", TRUE, ch->master,
          NULL, NULL, TO_ROOM);
        tAff.duration = ((Pulse::ONE_SECOND * 60) * 60) * ::number(12, 24);
        tAff.modifier2 = ::number(25, 75);
    ...
    case 8:
      ...
      act(
        "$n waves $s hands, utters many magic phrases and re-attaches your "
        "rod.",
        TRUE, mob, NULL, ch, TO_VICT);

      ch->setSex(SEX_MALE);

      break;
    default:
      vlogf(LOG_MOB_RS, format("Undefined response segment: %d") % which);
      break;
  }
