...
class TTerrainInfo {
  public:
    ...
    TTerrainInfo(int, int, int, int, int, int, int, int, const char*,
      const char*);
    ~TTerrainInfo();
...
// a typical waterskin holds 70 fl ounces, so pouring it overf your head is +70
// wetness a 0 means you dont dry or get any wetter. a general rating system:
//   100 - underwater
//   50 - most river surfaces (standing only. This number is doubled if you
//   sit/rest) 20 - wet.  a muddy swamp, puddles/mud etc. 10 - damp.  foggy, a
//   steamy jungle, etc 0 - what we would consider "damp". overcast day, wet air
//  -10 - what we could consider a 'regular' day.  most zones, dry cold, or cool
//  semi-humid -20 - a warm day in a temperate zone (most cities/indoors) -40 -
//  a desert
...
class TRoom : public TThing {
  ...
  public:
    ...
    void setRoomFlags(unsigned int flag) { roomFlags = flag; }
    void setTeleTarg(int targ) { teleTarg = targ; }
    void setTeleTime(int t_time) { teleTime = t_time; }
    void setMoblim(unsigned short limit) { moblim = limit; }
    void setRoomFlagBit(unsigned int i) { roomFlags |= i; }
    void removeRoomFlagBit(unsigned int i) { roomFlags = roomFlags & ~i; }
    bool isRoomFlag(unsigned int i) const { return ((roomFlags & i) != 0); }
    void clearRiverFields() {
      riverSpeed = 0;
      riverDir = DIR_NONE;
    }
    void setRiverDir(dirTypeT dir) { riverDir = dir; }
    void setRiverSpeed(int speed) { riverSpeed = speed; }
    void setRoomHeight(int r_height) { roomHeight = r_height; }
    zoneData* getZone() { return zone; }
    void setZoneNum(int z) { zone = &zone_table[z]; }
    int getZoneNum() const { return zone ? zone->zone_nr : -1; }
    unsigned short getMoblim() const { return moblim; }
    int getTeleTarg() const { return teleTarg; }
    short getTeleTime() const { return teleTime; }
    unsigned int getRoomFlags() const { return roomFlags; }
    int getXCoord() const { return x; }
    int getYCoord() const { return y; }
    int getZCoord() const { return z; }
    ...
    short getHasWindow() const { return hasWindow; }
    void setHasWindow(int window) { hasWindow = window; }
    short getTeleLook() const { return teleLook; }
    void setTeleLook(short look) { teleLook = look; }
    void decrementWindow() { hasWindow--; }
    void incrementWindow() { hasWindow++; }
    ...
    void playsound(soundNumT, const sstring&, int = 100, int = 50,
      int = 1) const;
