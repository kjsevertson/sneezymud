  ...
  if (!action_description.empty()) {
    ...
    if (ch->desc) {
      if (!ch->desc->m_bIsClient) {
        ...
        ch->sendTo(
          format("%s") % sb);  // tmpbuf may have "%" in it, do it this way
...
void TObj::show_me_to_char(TBeing* ch, showModeT mode) const {
  ...
  if (mode == SHOW_MODE_DESC_PLUS && !getDescr().empty()) {
    if (roomp && roomp->isWaterSector() && !isObjStat(ITEM_HOVER) &&
        !isObjStat(ITEM_FLOAT) && !(objVnum() == Obj::GENERIC_FLARE)) {
      buffer = format("%s is floating here.") % getName();
      buffer = buffer.cap();
    } else {
      capbuf = format("%s") %
               addNameToBuf(ch, ch->desc, this, getDescr(), COLOR_OBJECTS);
    ...
    if (isMonogrammed()) {
      ...
      sscanf(action_description.c_str(),
        "This is the personalized object of %s.", namebuf);
  ...
  if (!buffer.empty())
    buffer += "\n\r";
  buf =
    format("%s") % colorString(ch, ch->desc, buffer, NULL, COLOR_OBJECTS, TRUE);
  //  ch->sendTo(COLOR_OBJECTS, buffer);
  //  COSMO_COLOR
  ch->sendTo(buf);
}

void TObj::show_me_mult_to_char(TBeing* ch, showModeT mode,
  ...
  if (mode == SHOW_MODE_DESC_PLUS || mode == SHOW_MODE_SHORT_PLUS ||
      mode == SHOW_MODE_SHORT_PLUS_INV || mode == SHOW_MODE_TYPE ||
      mode == SHOW_MODE_PLUS) {
    buffer += showModifier(mode, ch);
...
bool TBeing::listThingRoomMe(const TBeing* ch) const {
  if (ch == this) {
    ...
    if (!ch->isAffected(AFF_SENSE_LIFE) && !ch->canSee(this, INFRA_YES) &&
      ...
      for (t2 = rider; t2; t2 = t2->nextRider) {
        ...
        if (ch == dynamic_cast<TBeing*>(t2) || ch->isAffected(AFF_SENSE_LIFE) ||
            ch->canSee(t2, INFRA_YES) || ch->canSee(t2)) {
          return true;
        }
      }
    }
  } else if (!ch->isAffected(AFF_SENSE_LIFE) && !ch->canSee(this, INFRA_YES) &&
             !ch->canSee(this)) {
    return true;
  }
  return false;
...
// returns true if should not be listed
bool TWindow::listThingRoomMe(const TBeing* ch) const { return true; }
...
bool list_in_heap_filtered(StuffList list, TBeing* ch, sstring filter,
  bool show_all, silentTypeT silent) {
  // used for applying a filter argument to an inventory list
  // for example, "inventory tool" will show only items matching "tool"

  // returns false if any items are found, so we know when to suppress the
  // "nothing" sendTo silent also flags any recursive instances to suppress the
  // "nothing" sendTo so only the first list_in_heap_filtered called will send a
  // "nothing" to the user, if nothing was found
  ...
  for (StuffIter it = list.begin(); it != list.end(); ++it) {
    ...
    if (show_all && !i->stuff.empty()) {
      suppress_nothing =
        list_in_heap_filtered(i->stuff, ch, filter, TRUE, SILENT_YES) ||
        suppress_nothing;
      ...
      if (dynamic_cast<TBaseContainer*>(i)) {
        ...
        if (!(oc && oc->isClosed())) {
          ...
          suppress_nothing =
            list_in_heap_filtered(i->stuff, ch, filter, FALSE, SILENT_YES) ||
            suppress_nothing;
        }
      }
    }
...
static sstring displayShowApprox(const TBeing* looked, const TBeing* looker,
  spellNumT tSkill, float tDiff) {
  ...
  // 0, (-6, ..., 6), ..., (-102, ..., 102), ..., 100
  tDeffer =
    max((float)100.0, min((float)0.0, (tDiff * (tDeffer + ::number(-2, 2)))));
  // 0, ..., 2
...
static void describeSpellEffects(const TBeing* me, const TBeing* ch,
  bool verbose) {
  char bufspell[1024];
  char bufpray[1024];
  int totspell = 0;
  int totpray = 0;
  sstring tStSpell(""), tStPray("");

  // these are the less common or more urgent affects that we want noticed
  if (me->getCaptiveOf()) {
    char buf[256];
    sprintf(buf, ".....$n is held captive by %s.",
      me->getCaptiveOf()->getName().c_str());
  ...
  if (me->affectedBySpell(AFFECT_PLAYERKILL)) {
    colorAct(COLOR_ROOMS,
      ".....$n is covered in <r>blood<1> from a recent murder!", FALSE, me, 0,
      ch, TO_VICT);
  }
  ...
  if (me->affectedBySpell(SPELL_CURSE)) {
    sprintf(bufpray,
      ".....$n is surrounded by a dark, forboding red aura!\n\r");
    tStPray += bufpray;
...
void TBeing::show_me_to_char(TBeing* ch, showModeT mode) const {
  ...
  if (mode == SHOW_MODE_DESC_PLUS) {
    if (!ch->canSee(this)) {
      if (ch->canSee(this, INFRA_YES)) {
        sprintf(buffer, "A blob of heat is here in the shape of %s %s%s.\n\r",
          ...
          getMyRace()->getSingularName().c_str(), isPkChar() ? " (PK)" : "");
        ch->sendTo(buffer);
    ...
    if (riding || spelltask || fight() ||
        ...
        getLongDesc().empty() || (tm && tm->getPosition() != tm->default_pos)) {
      ...
      if (hasColorStrings(NULL, getName(), 2)) {
        if (dynamic_cast<const TPerson*>(this))
          sprintf(buffer, "%s",
            colorString(ch, ch->desc, getName(), NULL, COLOR_MOBS, FALSE)
              .c_str());
        else
          sprintf(buffer, "%s",
            colorString(ch, ch->desc, sstring(getName()).cap().c_str(), NULL,
              COLOR_MOBS, FALSE)
              .c_str());
      } else
        sprintf(buffer, "%s%s%s", ch->cyan(), sstring(getName()).cap().c_str(),
          ch->norm());
        ...
        switch (getPosition()) {
          ...
          case POSITION_STANDING:
            if (roomp->isWaterSector())
              ...
              sprintf(buffer + strlen(buffer),
                " is standing here with a crazy glint in %s eye.", hshr());
            else
              strcat(buffer, " is standing here.");
            break;
      ...
      strcat(buffer, "\n\r");
      sprintf(buffer, "%s",
        colorString(ch, ch->desc, buffer, NULL, COLOR_MOBS, TRUE).c_str());
      ch->sendTo(buffer);
    } else {  // npc with long
      ...
      // long desc
      if (isPc() && (Strng.find(getName()) == sstring::npos) &&
          ch->isImmortal() &&
    ...
    if (!getDescr().empty()) {
      snprintf(capbuf, sizeof(capbuf), "%s",
        addNameToBuf(ch, ch->desc, this, getDescr(), COLOR_MOBS).c_str());
      sstring cStrbuf = capbuf;
      while (cStrbuf.find("$$g") != sstring::npos)
        cStrbuf.replace(cStrbuf.find("$$g"), 3,
          (roomp ? roomp->describeGround().c_str() : "TELL A GOD"));
    ...
    if (affectedBySpell(AFFECT_WET))
      act(format("$e looks %s.") % Weather::describeWet(this), FALSE, this,
        NULL, ch, TO_VICT);
    ...
    sprintf(buffer, "$e is about %d'%d\" tall and weighs around %d pounds.",
        ...
        if (equipment[ij] && ch->canSee(equipment[ij])) {
          ...
          sprintf(buf, "<%s>",
            (slot.find("Worn") != sstring::npos
                ? slot.replace(slot.find("Worn"), 4, "Tattooed").c_str()
                : slot.c_str()));
    ...
    if ((ch != this) && !ch->isImmortal() && ch->affectedBySpell(SKILL_SPY)) {
      ...
      const TMonster* monster =
        Config::LoadOnDeath() ? dynamic_cast<const TMonster*>(this) : NULL;
      ...
      if (Config::LoadOnDeath())
        ch->sendTo(COLOR_MOBS,
          format("You suspect their pockets contain %s.\n\r") %
            (success ? getStealLootNames() : "nothing"));
...
void TBeing::show_me_mult_to_char(TBeing* ch, showModeT,
  unsigned int num) const {
            ...
            if (!riding->getName().empty())
              ...
              if (dynamic_cast<TBeing*>(riding)) {
                ...
                if (IS_SET(ch->desc->plr_color, PLR_COLOR_OBJECTS)) {
                  sprintf(capbuf, "%s",
                    colorString(ch, ch->desc, ch->objs(riding), NULL,
                      COLOR_OBJECTS, FALSE)
                      .c_str());
...
sstring TStaff::getNameForShow(bool useColor, bool useName,
...
sstring TScroll::getNameForShow(bool useColor, bool useName,
  const TBeing* ch) const {
  char buf2[256];
  sprintf(buf2, "%s [%s/%s/%s]",
    useName ? name.c_str()
            : (useColor ? getName().c_str() : getNameNOC(ch).c_str()),
    (getSpell(0) >= 0
        ? (discArray[getSpell(0)] ? discArray[getSpell(0)]->name : "Unknown")
        : "None"),
    (getSpell(1) >= 0
        ? (discArray[getSpell(1)] ? discArray[getSpell(1)]->name : "Unknown")
        : "None"),
    (getSpell(2) >= 0
        ? (discArray[getSpell(2)] ? discArray[getSpell(2)]->name : "Unknown")
        : "None"));
