...
int TBeing::trapSpike(int amt, TTrap* trap) {
  ...
  if (equipment[limb]) {
    ...
    sstring buf = format("A spike tears through your %s, damaging it!") %
                  equipment[limb];
    act(buf, false, this, nullptr, nullptr, TO_CHAR, ANSI_YELLOW);
    buf = format("A spike tears through $n's %s, damaging it!") %
                  equipment[limb];
  ...
  sstring buf = format("A spike embeds itself in your %s!") %
...
int TBeing::trapBolt(int amt, TTrap* trap) {
  ...
  while (numLimbs > 0 && !validLimbs.empty()) {
    ...
    if (equipment[limb]) {
      auto* eq = dynamic_cast<TObj*>(equipment[limb]);
      eq->addToStructPoints(-limbDam);
      sstring buf = format("A bolt tears through your %s, damaging it!") %
                    equipment[limb];
      act(buf, false, this, nullptr, nullptr, TO_CHAR, ANSI_YELLOW);
      buf = format("A bolt tears through $n's %s, damaging it!") %
                    equipment[limb];
      act(buf, false, this, nullptr, nullptr, TO_ROOM, ANSI_YELLOW);
    }
    ...
    sstring buf = format("A bolt embeds itself in your %s!") %
                  describeBodySlot(limb);
    act(buf, false, this, nullptr, nullptr, TO_CHAR, ANSI_ORANGE);
    buf = format("A bolt embeds itself in $n's %s!") %
                  describeBodySlot(limb);
    act(buf, false, this, nullptr, nullptr, TO_ROOM, ANSI_ORANGE);
...
int TBeing::trapAcid(int amt, TTrap* trap) {
  ...
  while (numLimbs > 0 && !validLimbs.empty()) {
    ...
    sstring buf = format("Acid burns into your %s!") %
                  describeBodySlot(limb);
    act(buf, false, this, nullptr, nullptr, TO_CHAR, ANSI_GREEN_BOLD);
    buf = format("Acid burns into $n's %s!") %
                  describeBodySlot(limb);
...
int TBeing::trapExplosive(int amt, TTrap* trap) {
  ...
  while (numLimbs > 0 && !validLimbs.empty()) {
    ...
    sstring buf =
    format("Flaming chunks of shrapnel slam into $n's %s!") %
    describeBodySlot(limb);
  act(buf, false, this, nullptr, nullptr, TO_ROOM, ANSI_WHITE_BOLD);
  buf =
    format("Flaming chunks of shrapnel slam into your %s!") %
    describeBodySlot(limb);
  ...
  if (!isAgile(0)) {
    ...
    act("You are hit by debris, which embeds into your flesh!", false, this, 0,
      0, TO_CHAR);
    }
...
int TBeing::trapFrost(int amt, TTrap* trap) {
  ...
  for (wearSlotT limb = MIN_WEAR; limb < MAX_WEAR; limb++) {
    // Not sure of all the checks you'd need here
    if (hasPart(limb) && !equipment[limb] &&
        !getStuckIn(limb)) {
      validLimbs.push_back(limb);
    }
  }
...
int TBeing::trapFire(int amt, TTrap* trap) {
  ...
  for (wearSlotT limb = MIN_WEAR; limb < MAX_WEAR; limb++) {  // Fixed initialization
...
int TBeing::trapPower(int amt, TTrap* trap) {
  ...
  while (numLimbs > 0 && !validLimbs.empty()) {
    ...
    if (!this->equipment[limb]) {
      ...
      if (!this->isImmune(IMMUNE_ENERGY, limb) && (!this->isWise() || !this->isIntelligent())) {
  ...
  act("$N looks drained as the energy blasts $S!", false, this, nullptr, nullptr, TO_ROOM);
  act("You feel drained as the energy blasts you!", false, this, nullptr, nullptr, TO_VICT);
...
int TBeing::trapDisc(int amt, TTrap* trap) {
  ...
  while (numLimbs > 0 && !validLimbs.empty()) {
    ...
    if (equipment[limb]) {
      auto* eq = dynamic_cast<TObj*>(equipment[limb]);
      eq->addToStructPoints(-limbDam);
      sstring buf = format("A disc tears through your %s, damaging it!") %
                    equipment[limb];
      act(buf, false, this, nullptr, nullptr, TO_CHAR, ANSI_YELLOW);
      buf = format("A disc tears through $N's %s, damaging it!") %
                    equipment[limb];
      act(buf, false, this, nullptr, nullptr, TO_ROOM, ANSI_YELLOW);
    }
    addCurLimbHealth(limb, -limbDam);

    sstring buf = format("A sharp disc rips into $N's %s!") %
                  describeBodySlot(limb);
    act(buf, false, this, nullptr, nullptr, TO_ROOM, ANSI_ORANGE);
    buf = format("A sharp disc rips into your %s!") %
                  describeBodySlot(limb);
    act(buf, false, this, nullptr, nullptr, TO_CHAR, ANSI_ORANGE);
...
int TBeing::trapPebble(int amt, TTrap* trap) {
  ...
  while (numLimbs > 0 && !validLimbs.empty()) {
    ...
    if (this->equipment[limb]) {
      auto* eq = dynamic_cast<TObj*>(this->equipment[limb]);
      eq->addToStructPoints(-limbDam);
      sstring buf = format("A stone slams into $N's %s, damaging it!") %
                    equipment[limb];
      act(buf, false, this, nullptr, nullptr, TO_ROOM, ANSI_YELLOW);
      buf = format("A stone slams into your %s, damaging it!") %
                    equipment[limb];
      act(buf, false, this, nullptr, nullptr, TO_CHAR, ANSI_YELLOW);
    }
    addCurLimbHealth(limb, -limbDam*2);

    sstring buf = format("A pebble slams into $N's %s!") %
                  describeBodySlot(limb);
    act(buf, false, this, nullptr, nullptr, TO_ROOM, ANSI_ORANGE);
    buf = format("A pebble slams into your %s!") %
                  describeBodySlot(limb);
    act(buf, false, this, nullptr, nullptr, TO_CHAR, ANSI_ORANGE);

    /// do second part here within if statement
    if (isLimbFlags(limb, PART_BRUISED) && !isTough() && !isImmune(IMMUNE_BONE_COND, limb) && limbDam*4>=getCurLimbHealth(limb)) {
      addToLimbFlags(limb, PART_BROKEN);
      act("Your $s has been broken!", false, this, nullptr, nullptr,
        TO_CHAR, ANSI_RED_BOLD);
      act("$n's $s has been broken!", false, this, nullptr, nullptr,
        TO_ROOM, ANSI_RED_BOLD);
      } else if (!isImmune(IMMUNE_SKIN_COND, limb) && !isTough()) {
        addToLimbFlags(limb, PART_BRUISED);
        act("Your $s has been bruised!", false, this, nullptr, nullptr,
        TO_CHAR, ANSI_RED_BOLD);
      act("$n's $s has been bruised!", false, this, nullptr, nullptr,
        TO_ROOM, ANSI_RED_BOLD);
      }
