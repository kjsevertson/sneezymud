...
// affects for given applyTypeT
std::pair<int64_t, int64_t> affectedData::sumAffectsByApplyType(
  const applyTypeT affectType) const {
  int64_t mod1 = 0;
  int64_t mod2 = 0;
...
// Search a TBeing's affects, looking for one for which the predicate function
// returns true when passed the affect. If found, return a pointer to this
// affect.
affectedData* affectedData::find_if(
  const std::function<bool(affectedData*)>& predicate) {
