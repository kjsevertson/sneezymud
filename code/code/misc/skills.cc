...
#include "extern.h"
#include "disc_warrior_soldiering.h"
...
#include "disc_warrior_dueling.h"
...
#include "disc_cleric_faith.h"
#include "disc_advanced_slash.h"
#include "disc_advanced_blunt.h"
#include "disc_advanced_pierce.h"
#include "disc_advanced_ranged.h"
#include "disc_advanced_barehand.h"
...
short TBeing::getNatSkillValue(spellNumT skill) const {
  ...
  if (value < 0) {
    ...
    vlogf(LOG_BUG, format("%s showed up with 0 learning in a skill (%d)") %
                     getName() % skill);
    return SKILL_MIN;
  } else {
...
double TBeing::getSkillLearnednessPercent(const spellNumT skillNumber) const {
  const spellNumT skill = getSkillNum(skillNumber);
  if (getSkillValue(skill) == SKILL_MIN)
    return 0;
  return (static_cast<double>(getSkillValue(skill)) /
          static_cast<double>(MAX_SKILL_LEARNEDNESS));
}

CSkill::CSkill() : lastUsed(time(0)) {
  setLearnedness(SKILL_MIN);
  setNatLearnedness(SKILL_MIN);
}

CSkill::CSkill(const CSkill& a) : value(a.value), lastUsed(a.lastUsed) {}
...
int TBeing::durationModify(spellNumT skill, int duration) {
  ...
  if (discArray[skill]->modifierStat <= MAX_STATS_USED) {
    duration *=
      plotStat(STAT_CURRENT, discArray[skill]->modifierStat, 0.8, 1.25, 1.0);
  }
  return duration;
}
