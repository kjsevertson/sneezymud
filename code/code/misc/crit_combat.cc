...
// barehand crits. The crit table for pierce is slightly worse than blunt, so
...
// returns DELETE_THIS
// return ONEHIT_MESS_CRIT_S if oneHit should abort and no further oneHits
// should occur
    ...
    switch (num) {
      ...
      case 9:
        ...
        for (StuffIter it = roomp->stuff.begin(); it != roomp->stuff.end();
          ...
          if (!tbt->isAffected(AFF_CHARM) && inGroup(*tbt) && (tbt != this))
            break;
        }
        if (!tbt)
          tbt = this;
...
int TBeing::critSuccessChance(TBeing* victim, TThing* weapon,
  ...
  // allows more granular modifications of crit chance.
  static constexpr int pcCritRollRange = 100000;

  // Mob crit roll range is higher than PC by factor of 10, as we wants mobs
  ...
  static constexpr int baseCritSeverity = 10;

  // These values represent how much the respective skills increase the crit
  ...
  if (!isImmortal() &&
      (victim->isImmortal() || IS_SET(victim->specials.act, ACT_IMMORTAL)))
    return 0;

  if ((mod == -1) &&
      victim->isImmune(getTypeImmunity(weaponDamageType), *partHit))
    return 0;
  ...
  // Handle imm ego crit command affect
  ...
  // oneHit function. Things like the vorpal proc and the imm "crit" command
  // will send in a specific value for mod, which serves as the crit severity in
  // those cases.
  if (mod == -1) {
    ...
    // to allow all attackers the potential for the highest severity crits once
    // victim is at 10% or less of their maximum HP.

    // When attacker level > victim level, the level difference is added to the
    // base, meaning attacker can achieve more deadly crits immediately when
    // fighting a victim under its level.
    ...
    const int levelDifference = GetMaxLevel() - victim->GetMaxLevel();
    const int baseSeverity = levelDifference <= 0
                               ? baseCritSeverity
                               : baseCritSeverity + levelDifference;
    int maxCritSeverity = min(
      baseSeverity + static_cast<int>(victim->getPercentHpMissing() * 100.0),
      100);

    const int powerMoveBonus =
      static_cast<int>(powerMoveSeverityBonus * powerMoveValue / 100.0);
    const int critHittingBonus =
      static_cast<int>(critHittingSeverityBonus * critHittingValue / 100.0);
    maxCritSeverity += powerMoveBonus;
    maxCritSeverity += critHittingBonus;

    // To avoid penalizing players while fighting mobs in the "leveling mob"
    // range (arbitrarily decided as level 55), uncap crit severity in that
    // situation.
  ...
  if (slashType(weaponDamageType))
    return critSlash(victim, weapon, partHit, weaponDamageType, damage,
      critSeverity);
...
int TBeing::critBlunt(TBeing* v, TThing* weapon, wearSlotT* part_hit,
  ...
  if (crit_num > 100) {
    vlogf(LOG_BUG,
      format("critBlunt called with crit_num>100 (%i)") % crit_num);
    crit_num = 0;
  }
  ...
  if (crit_num <= 33) {
    ...
    *dam *= 3;
    ...
    switch (crit_num) {
      ...
      case 97:
        ...
        if (dynamic_cast<TPerson*>(this)) {
          ...
          buf = format("tooth lost limb %s [q] [tooth] [%d] [%d] [%s]") %
                v->name % limb_quest % v_vnum % getName();
        } else {
        ...
        buf = format(
                "<W>A <1><r>bloody<1><W> tooth lies here, having been knocked "
                "out of %s's mouth.<1>") %
              v->getName();
        corpse->setDescr(buf);

        corpse->obj_flags.wear_flags =
          ITEM_WEAR_TAKE | ITEM_WEAR_HOLD | ITEM_WEAR_THROW;
        //	corpse->addCorpseFlag(CORPSE_NO_REGEN);
        //	corpse->obj_flags.decay_time = 3 * (dynamic_cast<TMonster *>(this) ?
        // MAX_NPC_CORPSE_TIME : MAX_PC_CORPSE_EMPTY_TIME);
      ...
      case 100:
        if (doesKnowSkill(SKILL_CRIT_HIT) && !v->equipment[WEAR_BODY] &&
          ...
          if (IS_SET(v->specials.act, ACT_GHOST))
            ...
            if (dynamic_cast<TPerson*>(this))
              buf = format("heart lost limb %s [q] [heart] [0] [%d] [%s]") %
                    v->name % v_vnum % getName();
            else
            ...
            if (IS_SET(v->specials.act, ACT_ZOMBIE)) {
              ...
              buf = format("The putrid <r>heart<1> of %s lies here.") %
                    v->getName();
              corpse->setDescr(buf);
            } else {
              ...
              buf = format("The lifeless <r>heart<1> of %s lies here.") %
                    v->getName();
              corpse->setDescr(buf);
            }
            ...
            corpse->obj_flags.decay_time =
              3 * (dynamic_cast<TMonster*>(this) ? MAX_NPC_CORPSE_TIME
                                                 : MAX_PC_CORPSE_EMPTY_TIME);
            corpse->setWeight(0.1);
            corpse->canBeSeen = v->canBeSeen;
            corpse->setVolume(25);
      ...
      default:
        vlogf(LOG_BUG,
          format("crit_num=%i in critBlunt switch, shouldn't happen") %
            crit_num);
        break;
    }
  }
...
int TBeing::critSlash(TBeing* v, TThing* weapon, wearSlotT* part_hit,
  ...
  if (crit_num > 100) {
    vlogf(LOG_BUG,
      format("critSlash called with crit_num>100 (%i)") % crit_num);
    crit_num = 0;
  }
  if ((vmob = dynamic_cast<TMonster*>(v)))
    v_vnum = vmob->number >= 0 ? mob_index[vmob->getMobIndex()].virt : -1;
    ...
    switch (crit_num) {
      ...
      case 70:
        ...
        if ((obj = v->equipment[WEAR_WRIST_R])) {
          v->sendTo(COLOR_OBJECTS,
            format("Your %s saves you from losing your %s!\n\r") %
      ...
      case 73:
        ...
        if ((obj = v->equipment[WEAR_WRIST_L])) {
          v->sendTo(COLOR_OBJECTS,
            format("Your %s saves you from losing a hand!\n\r") %
              fname(obj->name));
          for (i = 1; i < 5; i++)
      ...
      case 80:
        ...
        if (v->roomp && !v->roomp->isRoomFlag(ROOM_ARENA)) {
          ...
          v->rawBleed(WEAR_LEG_R, PERMANENT_DURATION, SILENT_NO,
            CHECK_IMMUNITY_YES);
          *part_hit = WEAR_LEG_R;
          if (desc)
            desc->career.crit_sev_limbs++;
      ...
      case 82:
        ...
        if (v->roomp && !v->roomp->isRoomFlag(ROOM_ARENA) &&
          ...
          v->rawBleed(WEAR_LEG_L, PERMANENT_DURATION, SILENT_NO,
            CHECK_IMMUNITY_YES);
          *part_hit = WEAR_LEG_L;
          if (desc)
            desc->career.crit_sev_limbs++;
      ...
      case 86:
        if (v->roomp && !v->roomp->isRoomFlag(ROOM_ARENA) &&
            v->hasPart(WEAR_WAIST) && (obj = v->equipment[WEAR_WAIST])) {
          buf = format(
      ...
      case 90:
        ...
        if (v->roomp && !v->roomp->isRoomFlag(ROOM_ARENA) &&
            !IS_SET(v->specials.act, ACT_SKELETON)) {
          buf = format(
      ...
      case 91:
      case 92:
        if (v->getSex() == SEX_MALE && v->hasPart(WEAR_WAIST) &&
            (!(obj = v->equipment[WEAR_WAIST]) || !obj->isMetal()) &&
            !IS_SET(v->specials.act, ACT_SKELETON) &&
            !IS_SET(v->specials.act, ACT_GHOST)) {
          buf =
          ...
          if (dynamic_cast<TPerson*>(this))
            buf =
              format("genitalia lost limb %s [q] [jumblies] [0] [%d] [%s]") %
              v->name % v_vnum % getName();
          else
          ...
          if (v->getMaterial(WEAR_WAIST) > MAT_GEN_MINERAL) {
            // made of mineral or metal
            buf =
              format("The mangled, severed genitalia of %s is lying here.") %
              v->getName();
          } else {
            buf =
              format(
                "The bloody, mangled, severed genitalia of %s is lying here.") %
              v->getName();
          }
          corpse->setDescr(buf);
          ...
          *v->roomp += *corpse;
          ...
          v->setSex(SEX_NEUTER);
          v->rawBleed(WEAR_WAIST, PERMANENT_DURATION, SILENT_NO,
            CHECK_IMMUNITY_YES);
      ...
      default:
        vlogf(LOG_BUG,
          format("crit_num=%i in critSlash switch, shouldn't happen") %
            crit_num);
        break;
    }
  }
...
int TBeing::critPierce(TBeing* v, TThing* weapon, wearSlotT* part_hit,
  ...
  if (crit_num > 100) {
    vlogf(LOG_BUG,
      format("critPierce called with crit_num>100 (%i)") % crit_num);
    crit_num = 0;
  }
  ...
  if (crit_num <= 33) {
    ...
    *dam *= 3;
    ...
    // better stuff
    weaponStr =
      (weapon ? fname(weapon->name) : getMyRace()->getBodyLimbPierce(this));

    switch (crit_num) {
      case 67:
      case 68:
      case 69:
        if (!v->hasPart(WEAR_NECK) || IS_SET(v->specials.act, ACT_GHOST) ||
            IS_SET(v->specials.act, ACT_SKELETON))
          return 0;
        ...
        *part_hit = WEAR_NECK;
        if ((obj = v->equipment[WEAR_NECK])) {
          v->sendTo(COLOR_OBJECTS,
            format("Your %s saves you from a punctured larynx!\n\r") %
              fname(obj->name));
          for (i = 1; i < 5; i++)
      ...
      case 76:
        ...
        *dam *= 3;
        ...
        if ((obj = v->equipment[WEAR_LEG_R])) {
          v->sendTo(COLOR_OBJECTS,
            format("Your %s saves you from losing a tendon!\n\r") %
              fname(obj->name));
          for (i = 1; i < 5; i++)
      ...
      case 85:
        ...
        if ((obj = v->equipment[WEAR_BODY])) {
          v->sendTo(COLOR_OBJECTS,
            format("Your %s saves you from a punctured lung!\n\r") %
              fname(obj->name));
      ...
      case 87:
        ...
        *dam *= 3;
        ...
        if (IS_SET_DELETE(rc, DELETE_VICT))
          return DELETE_VICT;
        v->sendTo(
          "You won't be able to speak or breathe until you get that punctured "
          "lung fixed!!!\n\r");
        *part_hit = WEAR_BODY;
        if (desc)
      ...
      case 88:
        ...
        *dam *= 3;
        ...
        if ((obj = v->equipment[WEAR_BODY])) {
          v->sendTo(COLOR_OBJECTS,
            format("Your %s saves you from a kidney wound!\n\r") %
              fname(obj->name));
          for (i = 1; i < 7; i++)
            if (v->equipment[WEAR_BODY])
              v->damageItem(this, WEAR_BODY, wtype, weapon, *dam);
      ...
      case 93:
        ...
        if (v->hasPart(WEAR_WAIST))
          v->rawInfect(WEAR_WAIST, PERMANENT_DURATION, SILENT_NO,
            CHECK_IMMUNITY_YES);
        af.type = AFFECT_DISEASE;
      ...
      case 95:
        ...
        if ((obj = v->equipment[WEAR_BACK])) {
          v->sendTo(COLOR_OBJECTS,
            format("Your %s saves you from a gory wound!\n\r") %
              fname(obj->name));
          // triple damage
          *dam *= 3;
      ...
      default:
        vlogf(LOG_BUG,
          format("crit_num=%i in critPierce switch, shouldn't happen") %
            crit_num);
        break;
    }
  }
