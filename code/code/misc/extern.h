...
extern void bisect_arg_safe(const char*, int*, char*, unsigned int,
  const char* const array[]);
#define bisect_arg(in, nt, out, ar) \
  bisect_arg_safe(in, nt, out, cElements(out), ar)
...
extern int resize_personalize_object(TBeing* deity, TBeing*, int virt,
  int decay);
...
extern dirTypeT can_see_linear(const TBeing*, const TBeing* targ, int* rng,
  dirTypeT* dr);
...
extern void MakeRoomNoise(TMonster*, int room, const char* local_snd,
...
extern bool list_in_heap_filtered(StuffList list, TBeing* ch, sstring filter,
  bool show_all, silentTypeT silent = SILENT_NO);

extern bool pierceType(spellNumT);
extern bool bluntType(spellNumT);
extern bool slashType(spellNumT);
extern int generic_dispel_magic(TBeing*, TBeing*, int, immortalTypeT,
  safeTypeT = SAFE_NO);
extern int genericChaseSpirits(TBeing*, TBeing*, int, immortalTypeT,
  safeTypeT = SAFE_NO);
extern bool file_to_sstring(const char* name, sstring& buf,
  concatT concat = CONCAT_NO);
...
extern bool loadsetCheck(TBeing*, int, int, wearSlotT, const sstring&,
  resetFlag flags = resetFlagNone);
extern void room_iterate(TRoom*[],
  void (*func)(int, TRoom*, sstring&, struct show_room_zone_struct*), sstring&,
  void*);
...
extern int get_range_actual_damage(TBeing* ch, TBeing* victim, TObj* o, int dam,
  spellNumT attacktype);
extern void generic_sell(TBeing* ch, TMonster* keeper, TObj* obj, int shop_nr);
extern void generic_num_sell(TBeing* ch, TMonster* keeper, TObj* obj,
  int shop_nr, int num);
extern TRoom* room_find_or_create(int);
class TNote;
extern TNote* createNote(const sstring&);
...
extern int doLiqSpell(TBeing*, TBeing*, liqTypeT, int, bool ranged = false);
extern int doObjSpell(TBeing*, TBeing*, TMagicItem*, TObj*, const char*,
  spellNumT);
