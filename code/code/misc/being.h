...
class pointData {  // NOTE: pointdata is saved directly into charfile SO YOU CAN
                   // NOT CHANGE THIS SHIT
...
class equipmentData {
  ...
  public:
    TThing* operator[](int slot) const { return this->equipment[slot]; }
...
class TBeing : public TThing {
  ...
  public:
    ...
    short wimpy;
    bool delaySave;  // used as a hack to get around saving a char multiple
                     // times when doing things like sell all.commod
  ...
  public:
    ...
    const sstring getProfName() const;
    ...
    virtual immortalTypeT isImmortal(int = GOD_LEVEL1) const {
      return IMMORTAL_NO;
    }
    ...
    dirTypeT findDoor(const char*, const char*, doorIntentT,
      silentTypeT silent = SILENT_YES);
    ...
    virtual std::pair<bool, int> doPersonCommand(cmdTypeT, const sstring&,
      TThing*, bool) {
      return {};
    }
    ...
    void sendStatusGmcp() const;
    void sendPositionGmcp() const;
    ...
    void openUniqueDoor(dirTypeT, doorUniqueT, const char*, const char*,
    ...
    void playsound(soundNumT, const sstring&, int vol = 100, int prior = 50,
      int loop = 1);
    void playmusic(musicNumT, const sstring&, int vol = 100, int cont = 1,
      int loop = 1);
    void stopsound();
    void stopmusic();
    ...
    void doStrip(const char*);
    void doPlate(const char*);
    void doBangle(const char*);
    void doDistill(const char*);
    void doInfuse(const char*);
    void doTransmute(const char*);
    void doSew(const char*);
    ...
    void doRites(const char*);
    void doBolster(const char*);
    void doEnsoul(const char*);
    void doTailor(const char*);
    void doWeave(const char*);
    void doMine(const char*);
    ...
    int critSuccessChance(TBeing*, TThing*, wearSlotT*, spellNumT, int*,
      int = -1);
    ...
    void throwChar(TBeing* v, int to_room, bool throwerMove, silentTypeT silent,
      bool forceStand);
    ...
    bool canUseEquipment(const TObj*, silentTypeT,
      wearKeyT = WEAR_KEY_NONE) const;
    ...
    bool isDead() const { return (getHit() < -10); }
    bool willStun(int dam) const { return (getHit() - dam <= 0); }
    ...
    void setHeroNum(int num) { heroNum = num; }
    ...
    void dieFollower();
    int doDiscipline(spellNumT, const sstring&);
    ...
    int getTrainerPracs(const TBeing*, const TMonster*, classIndT, discNumT,
      int) const;
    int checkTrainDeny(const TBeing*, TMonster*, discNumT, int) const;
    int checkForPreReqs(const TBeing*, TMonster*, discNumT, classIndT,
      int) const;
    ...
    virtual void doFeedback(const sstring& type, int clientCmd,
      const sstring& arg);
  ...
  int doTripleKick(const char *, int, TBeing *);
    ...
    void doReturn(const sstring& argument, wearSlotT limb, bool tell,
      bool deleteMob = true);
    ...
    // Template functions
    template <typename T>
    T plotStat(statSetT whichSet, statTypeT whichStat, T minValue, T maxValue,
      T average, double power = 1.4) const {
