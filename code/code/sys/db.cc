...
const char* const File::NEWS_DIR = "txt/news.d";    /* directory of news entries */
...
const char* const File::WIZNEWS_DIR =
  "txt/wiznews.d"; /* directory of wiznews entries */
...
void runResetCmdE(zoneData& zd, resetCom& rs, resetFlag flags, bool&,
  ...
  if (mob->equipment[realslot]) {
    vlogf(LOG_LOW, format("'E' command operating on already equipped slot. "
                          "Zonefile #: %d; Command #: %d; Existing Item: %s") %
                     zd.bottom % rs.cmd_no %
  ...
  if (itemAsClothing && !mob->validEquipSlot(realslot) &&
      !itemAsClothing->isSaddle())
    vlogf(LOG_LOW,
...
resetCom::resetCommandId resetCom::getCommandId() {
  switch (command) {
    ...
    case 'I':
      return cmd_LoadObjEquippedProp;
    case 'J':
      return cmd_LoadObjSetLocalProp;
    case 'Y':
