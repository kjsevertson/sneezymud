...
TObj* generateStealLoot(TMonster* mob) {
  ...
  if (mob->hasClass(CLASS_MAGE | CLASS_SHAMAN)) {
    for (unsigned int cNum = 0; cNum < CompIndex.size(); cNum++) {
      ...
      if (comp == COMP_CELERITE || comp == COMP_CLARITY ||
          comp == COMP_SHADOW_WALK || comp == COMP_HASTE ||
          comp == COMP_FARLOOK || comp == COMP_INVISIBILITY ||
          comp == COMP_LEVITATE || comp == COMP_TRUE_SIGHT)
        percent = 50;
  ...
  // read_object(getRandomItem(cashMax), VIRTUAL) which will then retrieve for
  // weight that will be considerably more expensive but can we cache?
...
int failSteal(TBeing* thief, TBeing* victim, TObj* obj) {
  ...
  for (StuffIter it = thief->roomp->stuff.begin();
       it != thief->roomp->stuff.end();) {
    TThing* t = *(it++);
    if (!t)
      continue;
    ...
    if (!guard)
      continue;
    if (!guard->isPolice() || !guard->canSee(thief) || !guard->awake() ||
        guard == victim)
      continue;
...
// If you steal from a PC, we 'generate' gold (once only) for you to have.  This
// way stealing is a net positive
static int steal(TBeing* thief, TBeing* victim) {
  ...
  // generate money
  int money = Config::LoadOnDeath() && mob ? (int)mob->getLoadMoney()
                                           : victim->getMoney();
  ...
  if (mob && Config::LoadOnDeath() &&
    ...
    if (objLoot) {
      *thief += *objLoot;
      thief->sendTo(format("What's this?  You manage to nab %s that they were "
                           "carrying too!\n\r") %
                    objLoot->getName());
    }
  }
...
int TThing::stealModifier() { return 0; }
...
static int steal(TBeing* thief, TBeing* victim, const sstring& obj_name) {
  ...
  if (victim->awake()) {
    if (!thief->isImmortal() && eq_pos != WEAR_NOWHERE) {
      if ((eq_pos != WEAR_NECK && eq_pos != WEAR_FINGER_R &&
            eq_pos != WEAR_FINGER_L && eq_pos != WEAR_WRIST_R &&
            eq_pos != WEAR_WRIST_L) ||
          (victim->getPosition() <= POSITION_SLEEPING &&
            eq_pos != WEAR_FOOT_L && eq_pos != WEAR_FOOT_R &&
            eq_pos != WEAR_HAND_L && eq_pos != WEAR_HAND_R &&
            eq_pos != WEAR_HEAD)) {
        thief->sendTo(
  ...
  // The above was added to make steal a bit more realistic at Peel's request
  // --jh oh sure, blame me - peel
  ...
  if (!victim->awake() || is_imp ||
    ...
    if (is_imp || ((thief->getCarriedVolume() + obj->getTotalVolume()) <
...
int TBeing::doSteal(const sstring& argument, TBeing* vict) {
  ...
  if (obj_name.empty() || is_abbrev(obj_name, "coins") ||
      is_abbrev(obj_name, "talens"))
