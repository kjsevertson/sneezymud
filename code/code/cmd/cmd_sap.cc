...
int TBeing::doSap(const char* argument, TBeing* vict) {
  ...
  if (makesNoise() && victim->awake()) {
    ...
    act("You make too much noise, and $N hears you coming.", FALSE, this, 0,
      victim, TO_CHAR);
    act(
      "You hear $n's armor, and quickly attempt to dodge $s attempt to sap you.",
...
int TBeing::sapSuccess(TBeing* victim) {
  ...
  int dam = getSkillDam(victim, SKILL_SAP, skillLevel, getAdvLearning(SKILL_SAP));
  auto* weapon = dynamic_cast<TGenWeapon*>(heldInPrimHand());
  ...
  if (this->getPosition() >= POSITION_STANDING || this->isFlying()) {
    if (victim->getPosition() > POSITION_SITTING) {
      ...
      double heightDiff = (static_cast<double>(this->getHeight()) - victim->getHeight()) / (95.0 - 29.0);
      double multiplier = 1.0 + (heightDiff * 0.75);  // 0.75 at -1.0, 1.5 at 1.0
...
int TBeing::sapFail(TBeing* victim) {
  ...
  if (victim->getPosition() > POSITION_DEAD) {
    ...
    act("Your attempt to sap $N with your $p fails awkwardly.",
      FALSE, this, weapon, victim, TO_CHAR);
    act("$n attempts to sap you with $s $p but fails!", 
