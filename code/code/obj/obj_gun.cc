...
void TGun::dropSpentCasing(TRoom* roomp) {
  ...
  if (robj < 0 || robj >= (signed int)obj_index.size()) {
    vlogf(LOG_BUG,
      format("dropSpentCasing(): No object (%d) in database!") % 13874);
    return;
  }
...
void TBeing::doGload(sstring arg) {
  ...
  if (arg1 != "unload") {
    for (int i = 1; i <= 100; ++i) {
      generic_find(((sstring)(format("%i.%s") % i % arg1)).c_str(),
        FIND_OBJ_INV | FIND_OBJ_EQUIP | FIND_OBJ_ROOM, this, &tb, &bow);
    ...
    generic_find(arg2.c_str(), FIND_OBJ_INV | FIND_OBJ_EQUIP | FIND_OBJ_ROOM,
      this, &tb, &bow);
...
sstring TGun::statObjInfo() const {
  ...
  buf = format("Rate of Fire: %i, Ammo Type: %s, Ammo: %i, Ammo Loaded: %s") %
        getROF() % getAmmoDescr(getAmmoType()) %
        (ammo ? ammo->getRounds() : 0) % (ammo ? ammo->getName() : "None");
  sbuf += buf;
...
TGun::TGun() : TGenWeapon(), rof(1), ammotype(1) {
  setMaxSharp(100);
  setCurSharp(100);
}

TGun::TGun(const TGun& a) : TGenWeapon(a), rof(a.rof), ammotype(a.ammotype) {}
...
int TGun::shootMeBow(TBeing* ch, TBeing* targ, unsigned int count, dirTypeT dir,
  int shoot_dist) {
  TAmmo* ammo;
  TObj* bullet;
  char buf[256];
