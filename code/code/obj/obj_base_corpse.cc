  ...
  for (StuffIter it = this->stuff.begin(); it != this->stuff.end();) {
    ...
    if ((p = this->parent) || (p = this->equippedBy)) {
      *(p) += *t;
      ...
      if ((ttab = dynamic_cast<TTable*>(riding))) {
        ...
        *riding->roomp += *t;
        ...
        vlogf(LOG_BUG, format("Corpse (%s) decayed on %s, deleting %s") %
                         this->getName() % riding->getName() % t->getName());
        delete t;
      }
      ...
      *this->roomp += *t;
      ...
      vlogf(LOG_BUG, format("Unowned corpse decayed: %s - deleting: %s") %
...
void TBaseCorpse::setCorpseFlags(unsigned int f) { corpse_flags = f; }

unsigned int TBaseCorpse::getCorpseFlags() const { return corpse_flags; }

void TBaseCorpse::addCorpseFlag(unsigned int r) { corpse_flags |= r; }

void TBaseCorpse::remCorpseFlag(unsigned int r) { corpse_flags &= ~r; }

bool TBaseCorpse::isCorpseFlag(unsigned int r) const {
  return ((corpse_flags & r) != 0);
}

void TBaseCorpse::setCorpseRace(race_t r) { corpse_race = r; }

race_t TBaseCorpse::getCorpseRace() const { return corpse_race; }

void TBaseCorpse::setCorpseLevel(unsigned int r) { corpse_level = r; }

unsigned int TBaseCorpse::getCorpseLevel() const { return corpse_level; }

void TBaseCorpse::setCorpseVnum(int r) { corpse_vnum = r; }
...
int TBaseCorpse::objectDecay() {
  if (parent)
    ...
    if (getMaterial() == MAT_POWDER) {
      ...
      // check to see if corpse should do a 'last gasp' or whatever before
  ...
  if (dynamic_cast<TPCorpse*>(this))
    vlogf(LOG_MISC, format("PCorpse '%s' decayed in '%s' with %f exp.") %
...
void TBaseCorpse::getObjFromMeText(TBeing* tBeing, TThing* tThing,
  getTypeT tType, bool tFirst) {
  TPCorpse* tCorpse;
