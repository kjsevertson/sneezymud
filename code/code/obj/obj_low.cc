...
/*-------------------------------------------------------------------
  ...
  Here is how it works:
    ...
    Some points are weighted based on tier of the object (to skew a stat towards
 a class)

    We add in three categories for armor: struct, stats, main (armor for eq,
 damage for weapons) All of these point totals are calculated.  Then added. (min
 point total for any one is 1/2 the max)

    Add them all up, then determine the 'level' of the item based on a
 non-linear curve
 ------------------------------------------------------------------*/
...
sstring ObjectEvaluator::getTierString() {
  static const char* tiers[Tier_Max + 1] = {"Clothing", "Light Armor",
    "Medium Armor", "Heavy Armor", "Jewelry", "Common Weapon", "Simple Weapon",
    "Martial Weapon", "Unknown"};
...
int ObjectEvaluator::getPointValue(PointType pt) {
  ...
  // so you can't have swords with +100 mana and no dmg, or helmets with +100
  // str and no armor
  ...
  if (rawStat < 0)
    vlogf(LOG_LOW,
      format("ObjectEvaluator found negative stat points on %s (vnum %i)") %
        m_obj->getName() % m_obj->objVnum());
...
int ObjectEvaluator::getStatPointsRaw() {
  ...
  // used to be low_acPerLevel/(low_exchangeRate*.55)+bonus but we want to skew
  // to allow clothing to have more stats
  static float pointsPerStat[Tier_Max] = {
    ((low_acPerLevel * (low_exchangeRate * 0.55)) + 1.0) / 10.0,  // clothing
    ((low_acPerLevel * (low_exchangeRate * 0.65)) + (2.0 / 3.0)) /
      10.0,  // light
    ((low_acPerLevel * (low_exchangeRate * 0.75)) + (1.0 / 3.0)) /
      10.0,                                                     // medium
  ...
  for (int iAff = 0; iAff < MAX_OBJ_AFFECT; iAff++) {
    ...
    } else if (m_obj->affected[iAff].location == APPLY_SPELL ||
               m_obj->affected[iAff].location == APPLY_DISCIPLINE) {
      bonus = m_obj->affected[iAff].modifier2;
...
// note: at this point all point are adjusted to be uniform regardless of
// tier/slot/etc
double ObjectEvaluator::getLoadLevel(PointType type) {
  int points = getPointValue(type);
  const int free_points =
    140;  // the first N points don't count (establish newbie base)
  const double points_per_level = 7.0;  // how many points each level of eq gets
  const double level_of_diminishing_returns =
    35.0;  // after some levels we get diminishing returns
  const int points_of_diminishing_returns =
    int(points_per_level * level_of_diminishing_returns) + free_points;
  ...
  // calc the curve - if we were cool we'd properly handle points for partial
  // levels
  double level = level_of_diminishing_returns + 1.0;

  for (double point_total = points - points_of_diminishing_returns;
       point_total > 0 && level < 127.0; level += 1.0) {
    double scale = pow(scale_base,
      ((level - level_of_diminishing_returns) / level_of_diminishing_returns));
...
Tier WeaponEvaluator::getTier() {
  ...
  static unsigned int simpleFlags =
    ITEM_ANTI_SHAMAN | ITEM_ANTI_MAGE |
    ITEM_ANTI_MONK;  // clerics use simple weapons
  static unsigned int martialFlags =
    ITEM_ANTI_CLERIC |
    simpleFlags;  // ranger, thief, diekhan and warriors are heavy
